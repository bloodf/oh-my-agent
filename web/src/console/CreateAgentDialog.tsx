  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[calc(100svh-1rem)] overflow-y-auto sm:max-w-2xl">
        ...
        <form id="new-agent" className="grid gap-5" onSubmit={(event) => {
          ...
          <DialogFooter><Button type="button" variant="outline" className="h-9 px-4 font-bold" onClick={() => onOpenChange(false)}>Cancel</Button><Button id="new-agent-create" type="submit" className="h-9 px-4 font-bold bg-[var(--send)] text-white hover:bg-[var(--send-hover)]" disabled={busy}>{busy ? "Creating…" : `Create ${draft.kind}`}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
