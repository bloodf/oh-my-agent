...
- **Wire-up per worker mode.** RPC workers launch with all five synthetic user-root variables plus `PI_CODING_AGENT_DIR=<home>/.omp/agent`, blank default-profile selectors, `OMP_AUTH_BROKER_URL=<daemon gateway>`, and a per-worker `OMP_AUTH_BROKER_TOKEN`; their process cwd remains the assigned project for project `.omp/agents` discovery. The vault-wide upstream token remains daemon-only. In-process SDK sessions (`agentDir: <home>/.omp/agent`, shared `authStorage`/`modelRegistry`, private `AgentRegistry`) remain test/daemon-internal tooling, not production workers. Contract: `tests/contracts/discovery.contract.test.ts` proves a fresh process discovers canonical synthetic-home agents, keeps project precedence, and excludes plugin-private paths.
...
2. ~~Sandbox policy shipping~~ **Resolved.** One typed workspace-access policy is generated at spawn and compiled by platform adapters: macOS Seatbelt (`sandbox-exec`) and Linux `bwrap`; no copied per-project templates. `sandbox: true` performs a capability probe and fails closed when its platform adapter is unavailable. Generated policies bind the resolved workspace, materialized worker dir, OMP/Bun runtime paths, loopback gateway socket/port, and declared extra roots only; contract tests snapshot the generated arguments/profile without launching privileged containers.
