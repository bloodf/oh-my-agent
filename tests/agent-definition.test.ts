...
test("parsePeerDefinition — valid full definition returns typed PeerDefinition", () => {
	const content = buildAgentDoc(
		{
			...
			schedules: [
				{ cron: "0 9 * * 1", prompt: "Weekly review.", room: "#reviews" },
			],
			automations: [
				{ event: "pr.opened", prompt: "Auto-review.", room: "#reviews" },
			],
		},
	...
	expect(result.description).toBe(
		"Reviews PRs and posts findings to #reviews.",
	);
...
test("parsePeerDefinition — kebab-case read-summarize normalized to readSummarize property", () => {
	const content = buildAgentDoc(
		{
			name: "rs",
			description: "Read-summarize test.",
			spawns: ["a"],
			"read-summarize": true,
		},
...
test("parsePeerDefinition — duplicate unknown key still rejects", () => {
	...
	expect(() => parsePeerDefinition("/agents/dup.md", content)).toThrow(
		/Unknown key/,
	);
});
...
test("parsePeerDefinition — missing spawns → throws", () => {
	const content = buildAgentDoc(
		{ name: "norest", description: "No rest." },
		"You are unstoppable.",
	);

	expect(() => parsePeerDefinition("/agents/norest.md", content)).toThrow(
		/spawns/,
	);
});
...
test("parsePeerDefinition — empty spawns array → throws", () => {
	const content = buildAgentDoc(
		{ name: "static", description: "Static.", spawns: [] },
		"You are static.",
	);

	expect(() => parsePeerDefinition("/agents/static.md", content)).toThrow(
		/spawns/,
	);
});
...
test("parsePeerDefinition — relative workspace → throws", () => {
	...
	expect(() => parsePeerDefinition("/agents/rel.md", content)).toThrow(
		/workspace/,
	);
});
...
test("parsePeerDefinition — sandbox extraRoots relative → throws", () => {
	...
	expect(() => parsePeerDefinition("/agents/badroots.md", content)).toThrow(
		/sandbox/,
	);
});
...
test("parsePeerDefinition — rooms entry without # or @ → throws", () => {
	...
	expect(() => parsePeerDefinition("/agents/badtalk.md", content)).toThrow(
		/rooms/,
	);
});
...
test("parsePeerDefinition — unknown top-level key → throws", () => {
	...
	expect(() => parsePeerDefinition("/agents/stranger.md", content)).toThrow(
		/Unknown key/,
	);
});
...
test("parsePeerDefinition — unknown key mentions the key name in error", () => {
	...
	expect(() => parsePeerDefinition("/agents/alien.md", content)).toThrow(
		"rogueKey",
	);
});
...
test("parsePeerDefinition — nested unknown key inside wake → throws", () => {
	...
	expect(() => parsePeerDefinition("/agents/nosurprise.md", content)).toThrow(
		/Unknown key/,
	);
});
...
test("parsePeerDefinition — negative maxTurns → throws", () => {
	...
	expect(() => parsePeerDefinition("/agents/overkill.md", content)).toThrow(
		/autonomy/,
	);
});
	...
	const content = buildAgentDoc(
		{
			...
			spawns: ["a"],
			autonomy: { budgetUsd: -0.01 },
		},
		"You are expensive.",
	);

	expect(() => parsePeerDefinition("/agents/expensive.md", content)).toThrow(
		/autonomy/,
	);
});
...
test("fingerprintPeerDefinition — changes when body text changes", () => {
	...
	expect(fingerprintPeerDefinition(def1)).not.toBe(
		fingerprintPeerDefinition(def2),
	);
});
...
test("fingerprintPeerDefinition — changes when behavior field changes", () => {
	const def1 = parsePeerDefinition(
		"/a.md",
		buildAgentDoc(
			{ name: "x", description: "Desc.", spawns: ["a"], model: "@a" },
			"You are x.",
		),
	);
	const def2 = parsePeerDefinition(
		"/a.md",
		buildAgentDoc(
			{ name: "x", description: "Desc.", spawns: ["a"], model: "@b" },
			"You are x.",
		),
	);
...
test("fingerprintPeerDefinition — changes when workspace changes", () => {
	const def1 = parsePeerDefinition(
		"/a.md",
		buildAgentDoc(
			{ name: "x", description: "Desc.", spawns: ["a"], workspace: "/w1" },
			"You are x.",
		),
	);
	const def2 = parsePeerDefinition(
		"/a.md",
		buildAgentDoc(
			{ name: "x", description: "Desc.", spawns: ["a"], workspace: "/w2" },
			"You are x.",
		),
	);
...
test("fingerprintPeerDefinition — changes when spawns allowlist changes", () => {
	const def1 = parsePeerDefinition(
		"/a.md",
		buildAgentDoc(
			{ name: "x", description: "Desc.", spawns: ["a"] },
			"You are x.",
		),
	);
	const def2 = parsePeerDefinition(
		"/a.md",
		buildAgentDoc(
			{ name: "x", description: "Desc.", spawns: ["a", "b"] },
			"You are x.",
		),
	);
...
test("parsePeerDefinition — workspace non-string → throws", () => {
	...
	expect(() => parsePeerDefinition("/agents/bad.md", content)).toThrow(
		/workspace/i,
	);
});
...
test.each([
	...
	"parsePeerDefinition — autonomy invalid ($label) → throws",
	({ autonomy }) => {
		const content = buildAgentDoc(
			{
				name: "boundary",
	...
	const content = buildAgentDoc(
		{
			...
			description: "Boundary agent.",
			spawns: ["helper"],
			schedules: [{ cron: "*/5 9-17 * * 1,2", prompt: "Weekday standups" }],
		},
		"You are correct.",
	);
	...
	const content = buildAgentDoc(
		{
			...
			description: "Boundary agent.",
			spawns: ["helper"],
			automations: [{ event: "", prompt: "do it" }],
		},
		"You are wrong.",
	);
	expect(() => parsePeerDefinition("/agents/bad.md", content)).toThrow(
	...
	const content = buildAgentDoc(
		{
			...
			spawns: ["helper"],
			automations: [
				{ event: "on-file-change", prompt: "react", room: "no-hash" },
			],
		},
		"You are wrong.",
	);
...
test("parsePeerDefinition — body whitespace only → throws", () => {
	const content = buildAgentDoc(
		{ name: "boundary", description: "Boundary agent.", spawns: ["helper"] },
		"   \n\t  \n",
	);
