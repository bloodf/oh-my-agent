 ...
 */
async function withSyntheticHome(
...
describe("discoverAgents public API", () => {
	...
	test("returns { agents: AgentDefinition[], projectAgentsDir }", async () => {
		...
		expect(
			result.projectAgentsDir === null ||
				typeof result.projectAgentsDir === "string",
		).toBe(true);
	});
});
...
describe("projectAgentsDir points at project root", () => {
	test("equals the resolved .omp/agents path of the project cwd", async () => {
		const proj = await withTempProject({
			"proj-agent.md": makeAgentMd("proj-agent"),
		});
...
describe(".omp/<plugin>/agents is NOT a discovery root", () => {
	test("agent under .omp/oh-my-agent/agents is not discovered", async () => {
		...
		await writeFile(
			join(privateDir, "private-agent.md"),
			makeAgentMd("private-agent"),
			"utf8",
		);
		...
		expect(
			result.agents.find((a) => a.name === "private-agent"),
		).toBeUndefined();
		await cleanup(proj);
	});
	...
	test("public .omp/agents agent is discovered; plugin-private is not", async () => {
		...
		await writeFile(
			join(publicDir, "public-agent.md"),
			makeAgentMd("public-agent"),
			"utf8",
		);
...
describe("agent file format requirements", () => {
	test("valid markdown agent with frontmatter is discovered", async () => {
		const proj = await withTempProject({
			"valid.md": makeAgentMd("valid-agent"),
		});
		const result = await discoverAgents(proj);
	...
	test("file without frontmatter is silently skipped (no throw)", async () => {
		const proj = await withTempProject({
			"no-frontmatter.md": "You are a plain agent.",
		});
		const result = await discoverAgents(proj);
		expect(
			result.agents.find((a) => a.name === "no-frontmatter"),
		).toBeUndefined();
		await cleanup(proj);
	});
});
...
//
// Child environment (built by hermeticChildEnv, which first scrubs every inherited
// config-root selector — PI_CONFIG_DIR, CLAUDE_CONFIG_DIR, PI_CODING_AGENT_DIR,
// OMP_PROFILE/PI_PROFILE(+_DIR) — so host shell exports cannot reroute discovery):
...
describe("subprocess: synthetic worker — canonical layout", () => {
	test("user-only present; project duplicate wins; plugin-private absent; exact projectAgentsDir", async () => {
		...
		try {
			...
			const childEnv = hermeticChildEnv({
				HOME: homeEscaped,
				XDG_CONFIG_HOME: join(homeEscaped, ".config"),
				XDG_DATA_HOME: join(homeEscaped, ".local", "share"),
				XDG_STATE_HOME: join(homeEscaped, ".local", "state"),
				XDG_CACHE_HOME: join(homeEscaped, ".cache"),
				PI_CODING_AGENT_DIR: join(homeEscaped, ".omp", "agent"),
				OMP_PROFILE: "",
				PI_PROFILE: "",
			});
			...
			if (!dup)
				throw new Error("duplicate agent missing from discovery result");
