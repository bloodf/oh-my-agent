...
export class AccountStateMachine {
  ...
  updateMeter(value: number): void {
    ...
    if (value < 0 || value > 1) throw new Error("Meter value must be between 0 and 1");
    this.meter = value;
  }
  ...
  applyBlock(block: QuotaBlock): number {
    ...
    if (this.dep.mode === "subscription" && !this.parked && this.runs.size > 0) {
  ...
  clearBlock(credentialId: number, providerKey: string, scope: string): void {
    ...
    this.blocks = this.blocks.filter(
      (b) =>
        !(b.credentialId === credentialId && b.providerKey === providerKey && b.scope === scope),
    );
  }
  ...
  bumpBudget(meter: number): void {
    ...
    if (meter < 0 || meter > 1) throw new Error("Meter must be between 0 and 1");
