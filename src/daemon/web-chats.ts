 ...
 * session JSONL, and launch shim. A storage path pre-created by someone else
 * is never used; chats move to a fresh private directory beside it.
 *
 ...
 * both sources the same way. When the shim has also recorded the OMP process
 * it started, that process must be alive too: the shim can outlive a crashed
 * OMP only briefly, and OMP can outlive a SIGKILLed shim.
 *
...
export function chatAlive(client: object, pidPath: string): boolean {
	...
	const recorded = recordedPids(pidPath);
	const pid = (client as { pid?: number }).pid ?? recorded.shim;
	if (pid === undefined || !pidAlive(pid)) return false;
	return recorded.omp === undefined || pidAlive(recorded.omp);
}
...
/** The shim's pid on the first line, the OMP process group's on the second. */
function recordedPids(pidPath: string): { shim?: number; omp?: number } {
	let lines: string[];
	try {
		lines = readFileSync(pidPath, "utf8").split("\n");
	} catch {
		return {};
	}
	const parse = (line: string | undefined): number | undefined => {
		const value = Number.parseInt(line?.trim() ?? "", 10);
		return Number.isInteger(value) && value > 0 ? value : undefined;
	};
	return { shim: parse(lines[0]), omp: parse(lines[1]) };
}

/**
 * Stop a chat's client and make sure the OMP process group it ran goes too.
 *
 * The client stops the shim it spawned along with that shim's descendants.
 * Once the shim is gone (OOM killer, a stray kill -9) OMP is reparented, so
 * that walk no longer reaches it: OMP kept running, still held the client's
 * output pipes so the stop never finished, and a relaunch started a second
 * OMP on the same session file.
 *
 * ponytail: the group id comes from the pid file, so a group recycled in the
 * moment after it exited could be signalled. Upgrade to a pidfd if it matters.
 */
export async function stopChatProcess(
	client: { stop(): Promise<void> },
	pidPath: string,
): Promise<void> {
	// While the shim lives, the client's own tree walk reaches OMP.
	const { shim, omp } = recordedPids(pidPath);
	if (
		omp !== undefined &&
		(shim === undefined || !pidAlive(shim)) &&
		process.platform !== "win32"
	) {
		try {
			process.kill(-omp, "SIGKILL");
		} catch {
...
/**
 * The source of a per-chat launch shim: run the OMP CLI as the leader of its
 * own process group with the client's arguments and environment, record both
 * pids, and forward the signals the client uses to stop it to that group.
 */
function chatShimSource(cliPath: string, pidPath: string): string {
	return [
		"// Generated by oh-my-agent. Rebuilt on every launch; edits are discarded.",
		`const child = Bun.spawn(["bun", ${JSON.stringify(cliPath)}, ...process.argv.slice(2)], {`,
		"	detached: true,",
		'	stdin: "inherit",',
		'	stdout: "inherit",',
		'	stderr: "inherit",',
		"});",
		`await Bun.write(${JSON.stringify(pidPath)}, \`\${process.pid}\\n\${child.pid}\`);`,
...
interface LiveChat {
	...
	/** Where this chat's launch shim recorded the process pid. */
	pidPath: string;
}
...
/**
 * Claim the per-instance storage directory under the shared temporary root.
 *
 * The name is predictable so chats survive a daemon restart, which also lets
 * another local user create it first as a file, a symlink, or a directory
 * they own. Such a path is never used: the chats move to a fresh private
 * directory beside it instead of failing boot.
 */
async function privateStorageDir(preferred: string): Promise<string> {
	try {
		await mkdir(preferred, { mode: DIRECTORY_MODE });
	} catch (error) {
		if ((error as NodeJS.ErrnoException).code !== "EEXIST") throw error;
	}
	const info = await lstat(preferred);
	const mine =
		typeof process.getuid !== "function" || info.uid === process.getuid();
	if (!info.isDirectory() || !mine) return await mkdtemp(`${preferred}-`);
	await chmod(preferred, DIRECTORY_MODE);
	return preferred;
}
...
export async function createWebChats(
	...
	const storageDir = await privateStorageDir(
		join(tmpdir(), `oh-my-agent-web-${owner}-${instanceHash}`),
	);
	const sessionDir = join(storageDir, "sessions");
	// Per-chat launch shims and the pids they record, beside the sessions.
	const shimDir = join(storageDir, "shims");
	await mkdir(shimDir, { recursive: true, mode: DIRECTORY_MODE });
	const chatPidPath = (id: string): string => join(shimDir, `${id}.pid`);
	...
	const launch = async (record: PersistedChat): Promise<LiveChat> => {
		...
		try {
			...
			unsubscribe();
			await stopChatProcess(client, pidPath).catch(() => {});
			throw error;
		}
		...
		const watchExit = (): void => {
			const timer = setTimeout(() => {
				if (chatAlive(client, chatPidPath(record.id))) {
					watchExit();
					return;
				}
				...
				unsubscribe();
				// Reap an OMP that outlived its shim before anything relaunches it.
				void stopChatProcess(client, pidPath).catch(() => {});
			}, 1_000);
			timer.unref();
		};
		watchExit();
	...
	const start = async (record: PersistedChat): Promise<LiveChat> => {
		...
		if (existing && chatAlive(existing.client, chatPidPath(record.id)))
			return existing;
		if (existing) {
			existing.unsubscribe();
			live.delete(record.id);
			await stopChatProcess(existing.client, existing.pidPath).catch(() => {});
		}
	...
	const mutate = async <T>(
		...
		const guarded = async (): Promise<T> => {
			if (closed || !chatAlive(chat.client, chat.pidPath))
	...
	return {
		...
		create: async (input: CreateWebChatInput) => {
			...
			try {
				...
				chat?.unsubscribe();
				if (chat)
					await stopChatProcess(chat.client, chat.pidPath).catch(() => {});
				await persist();
				throw error;
			}
		...
		state: async (id) => {
			...
			return {
				...publicInfo(record),
				running: chatAlive(chat.client, chat.pidPath),
		...
		closeChat: async (id) => {
			...
			if (chat) {
				live.delete(record.id);
				chat.unsubscribe();
				await stopChatProcess(chat.client, chat.pidPath);
			}
			await persist();
			// A closed chat cannot be reopened, so its transcript and launch
			// files are only disk use. The session path is revalidated so corrupt
			// metadata cannot aim this removal outside session storage.
			await Promise.all([
				...(record.sessionFile === undefined
					? []
					: [rm(sessionPath(record.sessionFile), { force: true })]),
				rm(join(shimDir, `${record.id}.ts`), { force: true }),
				rm(chatPidPath(record.id), { force: true }),
			]);
		},
		close: async () => {
			if (closed) return;
