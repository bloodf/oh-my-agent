...
export interface InferenceGateway {
	...
	token: string;
	/** Resolved server-side output ceiling, applied after model discovery. */
	maxTokens?: number;
}
 ...
 *
 * Provider overrides preserve `transport` when discovery creates live rows, so
 * every discovered model remains pi-native and routes through `/v1/pi/stream`.
 */
...
export async function materializeWorker(
	...
	try {
		...
		await writeFile(
			...
			renderModels(inferenceGateway, provider, modelId),
			"utf8",
		);
