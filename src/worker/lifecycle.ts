 ...
 */
function shimSource(argv: string[], pidPath: string): string {
	return [
		"// Generated by oh-my-agent. Rebuilt on every spawn; edits are discarded.",
		`const argv = ${JSON.stringify(argv)};`,
		"// This shim is the process `RpcClient` spawns, so its own pid is the",
		"// worker's pid. Recorded here because the client exposes no accessor",
		"// for it in a released version, and a patch only applies in a checkout",
		"// that declares it — never in a consumer's install.",
...
async function writeEnvShim(
	...
	await writeFile(
		shimPath,
		shimSource(["bun", entry], workerPidPath(layout)),
		"utf8",
	);
	return shimPath;
}
...
async function writeSandboxShim(
	...
	await writeFile(
		shimPath,
		shimSource([plan.command, ...plan.args], workerPidPath(layout)),
		"utf8",
	);
	return shimPath;
}

/** Where a launched shim records the worker process's pid. */
function workerPidPath(layout: WorkerLayout): string {
	return join(layout.root, "worker.pid");
}
...
export function buildWorkerPolicy(
	...
	return {
		...
		// Derived from the artifacts actually executed, not a fixed FHS list.
		// `import.meta.dir/..` is this package's `src`, so the OMP CLI the shim
		// execs — `node_modules/@oh-my-pi/pi-coding-agent/dist/cli.js` — sat
		// outside every readable subpath under a `(deny default)` profile, as
		// did the `bun` binary running it. The gate probed clean and the
		// grandchild then died on a read denial.
...
/**
 * Read-only roots a sandboxed worker needs to execute at all: the system
 * runtime, the `bun` binary running the child, and the installed OMP package.
 *
 * Duplicates are collapsed and unresolvable entries dropped — a policy naming
 * a path that does not exist is refused by the compiler, and a missing `bun`
 * on PATH is a launch failure the gate reports on its own terms.
 */
export function sandboxRuntimePaths(): string[] {
	const roots = ["/usr/bin", "/bin", "/usr/lib", "/System", "/opt/homebrew"];
	const bun = Bun.which("bun");
	if (bun !== null) roots.push(dirname(bun));
	try {
		// `<package root>/dist/cli.js` — two levels up is the package itself,
		// which also carries the runtime files the CLI reads beside it.
		roots.push(dirname(dirname(resolveOmpCli())));
	} catch {
		// An unresolvable CLI is not this function's failure to report: the
		// launch path raises it by name a moment later.
	}
	return [...new Set(roots)].filter((path) => existsSync(path));
}
...
async function gatePeer(
	...
	// The peer's own acceptance of a filesystem-only sandbox, which the Linux
	// adapter requires before it will compile one. Without threading it, every
	// opted-in peer on Linux failed closed with `SANDBOX_NETWORK_UNENFORCED`
	// and no definition could say otherwise.
	const allowUnenforcedNetwork =
		typeof peer.sandbox === "object" &&
		peer.sandbox.allowUnenforcedNetwork === true;
...
export async function startWorker(
	...
	const launch = async (): Promise<void> => {
		...
		// Cleared before the shim can write a new one: a file left from the
		// previous launch would otherwise report a dead pid as this worker's
		// for the whole window between spawning and the shim starting up.
		await rm(workerPidPath(layout), { force: true });
		...
		get pid() {
			...
			try {
				const recorded = Number.parseInt(
					readFileSync(workerPidPath(layout), "utf8").trim(),
					10,
				);
				...
				// The shim has not written it yet, or this worker never got one.
				return undefined;
			}
		},
		layout,
