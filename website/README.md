...
- **Facts.** Copy and numbers come from the repository docs. `components/home/data.ts` notes the source of every count (17 CLI verbs from `docs/guide/cli.md`, 59 palettes from `web/DESIGN.md`, and so on). The demo password is read from `src/mock/demoPassword.ts`.
...
- **Login.** `/console` redirects to `/console/login` until the session holds the demo password, `melon` (the one constant in `src/mock/demoPassword.ts`). The login page reuses the console's `AuthScreen` markup and ids. It writes `sessionStorage["oh-my-agent.operator-token"]`, the same key the console reads its operator token from, so every `/api/*` call carries `X-Operator-Token: melon`; a call without it gets `401 Operator token refused`.
...
- **Banner.** `src/components/demo/DemoBanner.tsx` marks both pages as a demo and offers **Reset demo data** (clears the `oh-my-agent-demo:` keys and reloads) and **Log out**. The Lavish **Open review** links go to a stand-in at `/console/review/[id]`.
