...
ADRS += [
    ...
    ADR(
        ...
        decision=(
            ...
            "patched in the browser, state is seeded from fixtures and persisted in localStorage, and the demo "
            "operator token is written into the session before the console mounts, so there is no login. Nothing "
    ...
    ADR(
        id="ADR-018",
        slug="remove-server-rendered-console",
        title="Remove the server-rendered console",
        status="Accepted",
        context=(
            "ADR-016 added web-next/ beside the bundled console. The September 2026 review found its proxy attached "
            "the operator token to every /api request with no authentication, origin, or host check while Next listened "
            "on every interface, so anyone on the network or any web page could drive the daemon. It also cached a stale "
            "daemon port, mirrored wire types by hand, and lacked threads, chats, changes, definition editing, and remote auth."
        ),
        decision=(
            "Delete web-next/ and its suite, scripts, CI steps, and docs. The daemon's bundled console is the only console, "
            "and the lazy chunks from T-1630 keep its first load small."
        ),
        consequences=[
        ...
        evidence=[
            ("The review that found the proxy exposure", "docs/develop/review-2026-09.md"),
            ("CI without the Next console", ".github/workflows/ci.yml"),
        ],
    ),
    ADR(
        id="ADR-019",
        slug="blobatar-default-avatars",
        title="Default avatars are in-process blobatars from the wire name",
        status="Accepted",
        context=(
            "An empty profile drew two-letter initials on a hashed tint. The operator asked for "
            "https://github.com/Alain00/blobatar as the default face for every author."
        ),
        decision=(
            "Generate blobatars in-process from the wire identity (`@you`, a peer name) with the "
            "`blobatar` package. Stored emoji glyphs and uploaded images remain optional overrides. "
            "A ⏳ reaction on a loaded message puts that actor's default blobatar in the thinking pose. "
            "The TUI paints the blobatar body fill as a colored disc. Never call blobatar.dev."
        ),
        consequences=[
        ...
        evidence=[
            ("Empty stored avatar is a blobatar of the wire author", "tests/avatar.test.ts"),
            ("Browser: default face is a blobatar; a glyph still wins", "tests/console-client.test.ts"),
        ],
    ),
]
...
EPICS = [
    ...
    Epic(
        ...
        adrs=["ADR-014", "ADR-015", "ADR-016", "ADR-019"],
    ),
    Epic(
        id="EP-17",
        slug="public-site",
        ...
        acceptance=[
            ...
            "The demo opens without a login and still exercises the console's operator-token path.",
    ...
    Epic(
        id="EP-18",
        slug="review-2026-09",
        title="September 2026 review fixes",
        outcome=(
            "Every P0 and P1 finding from the September 2026 review is fixed and guarded by a test that fails "
            "without the fix, so the next round of work is features rather than repairs."
        ),
        why=(
            "The suite was green while a real daemon broke reloads and Mermaid, rooms froze past 500 messages, Plans "
            "lost edits on conflict, web-next exposed the operator token, and the website and demo mock sat outside CI."
        ),
        scope=["The tasks in this epic."],
        non_goals=[
            "New features.",
            "Live-account dogfooding (T-1403) and the upstream resolve walk (T-1503).",
        ],
        acceptance=[
            "The browser suite loads the console through the daemon's own static path.",
            "Every finding in docs/develop/review-2026-09.md is fixed or recorded with a reason.",
            "CI gates the website build, the demo mock smoke, and mock parity with the daemon.",
        ],
        adrs=["ADR-018"],
    ),
]
...
SPRINTS = [
    ...
    Sprint(id="SP-19", slug="review-fixes", title="Review fixes",
           theme="Fix what the September 2026 review found and make each failure visible to CI."),
]
...
TASKS += [
    ...
    Task(
        ...
        files=["src/extension/index.ts", "src/extension/commands.ts", "src/extension/widget.ts", "src/extension/ensure-daemon.ts", "src/extension/cli.ts", "tests/extension.test.ts", "tests/ensure-daemon.test.ts"],
        assets=[
            ...
            ("src/extension/ensure-daemon.ts", "New", "Session-start auto-start of the detached daemon from the plugin tree, not PATH."),
            ("src/extension/cli.ts", "New", "`/cli` and `/console` run `runCli` in-process so PATH is never required."),
            ("tests/extension.test.ts", "New", "Command output and no-daemon degradation."),
            ("tests/ensure-daemon.test.ts", "New", "Probe no-op when up; spawn when down; injected spawn seam."),
...
TASKS += [
    ...
    Task(
        ...
        assets=[
            ...
            ("src/daemon/workspace-changes.ts", "New", "Bounded read-only Git inspection with external helper protection."),
            ("src/rooms/plans.ts", "New", "Durable revision-controlled room plans."),
        ...
        steps=[
            ...
            "Build the split React/shadcn component tree into exactly the daemon's three allowlisted static files.",
        ],
        acceptance=[
        ...
        evidence=[
            ("Conversation-first React client built for the daemon", "src/console/app.js"),
            ("Browser-driven room, auth, keyboard, and reconnect contracts", "tests/console-client.test.ts"),
        ],
        depends_on=["T-602"],
    ),
    Task(
...
TASKS += [
    ...
    Task(
        ...
        steps=[
            "Author the files allowlist: src, skills, patches, LICENSE, README — skills/ is load-bearing (the materializer throws Unknown skill without it) and is in the manifest today. Tests, docs, and .github stay out. patches/ stays in the allowlist for developers installing from a checkout; whether npm actually packs the patched file varies by npm version, and the test treats both outcomes as acceptable.",
        ...
        acceptance=[
            "The dry-run manifest contains src/ (the src/console/*.html/css/js assets included), skills/ with every SKILL.md, LICENSE, and README.md, and nothing under tests/, docs/, or .github/. Under patches/ only the one patched file may appear, and its presence is npm's choice rather than a contract: npm 12 strips the file a patchedDependencies entry names while npm 11 packs it, and per ADR-013 it cannot take effect for a consumer either way.",
    ...
    Task(
        ...
        assets=[
            ("src/defaults/agents/staff-pm.md", "New", "Staff product manager: scope, sequencing, acceptance criteria; #product and #team."),
            ("src/defaults/agents/staff-backend.md", "New", "Staff backend engineer: services, data, APIs, tests with non-vacuity; #backend and #team."),
            ("src/defaults/agents/staff-frontend.md", "New", "Staff frontend engineer: accessible, fast interfaces against the published API; #frontend and #team."),
            ("src/defaults/agents/staff-qa.md", "New", "Staff QA engineer: verifies every criterion against the real system; #qa and #team."),
            ("src/daemon/default-peers.ts", "New", "Seeds the shipped peers into the user store once; a marker keeps edits and deletions."),
            ("src/daemon/default-model.ts", "New", "OMP's default model role as a provider/id, thinking suffix dropped."),
            ("src/daemon/runtime.ts", "Edited", "Seeds on the launcher's boot, hands the default to a model-less worker on both paths, reports the effective model, and lists the catalog."),
            ("src/daemon/inference-gateway.ts", "Edited", "listRoutableModels: the registry the scoped gateway builds, as a catalog."),
            ("src/shared/protocol.ts", "Edited", "models_list, additive."),
            ("src/shared/protocol-schemas.ts", "Edited", "models_list validators."),
            ("src/daemon/socket.ts", "Edited", "models_list handler over an optional context capability."),
            ("src/daemon/cli.ts", "Edited", "omp-agent models."),
            ("src/daemon/console-api.ts", "Edited", "GET /api/models."),
            ("src/extension/commands.ts", "Edited", "The model picker lists the catalog with the default marked and stores the bare selector."),
            ("web/src/console/CreateAgentDialog.tsx", "Edited", "A datalist of the catalog and the default as the placeholder."),
            ("tests/default-peers.test.ts", "New", "The shipped definitions parse; seeding is offered once; the default model resolves from config.yml."),
        ],
        steps=[
            "Write the four definitions with `spawns: \"*\"` and no model, so nothing depends on a task agent or a provider the operator has not set up.",
            "Seed them once per user store from the real launcher only, recording every name offered so an edit or a deletion survives the next boot.",
            "Resolve OMP's default model role at boot and hand it to the worker factory only for a peer that declares none; a declared model is never overridden.",
            "Expose the catalog the scoped gateway already knows how to build as `models_list`, and offer it from every surface that edits a model.",
        ],
        acceptance=[
            "A launcher boot of an empty store lists the four staff peers running, each on the OMP default, and `agents` names that model.",
            "A second boot seeds nothing; an edited or deleted seed is left as the operator left it.",
            "`models_list` answers the catalog and the default; the TUI picker offers it and stores the bare selector when the marked default is chosen.",
        ],
        evidence=[
            ("Shipped definitions parse; seeding once; edits and deletions kept; default model from config.yml", "tests/default-peers.test.ts"),
            ("Seeded peers boot on the default; a declared model is not overridden; models_list over the real socket", "tests/daemon-main.test.ts"),
            ("The catalog names what the credential scope can reach", "tests/inference-gateway.test.ts"),
            ("The picker lists the catalog and stores the bare selector", "tests/extension.test.ts"),
            ("omp-agent models in text and JSON", "tests/daemon-cli.test.ts"),
            ("GET /api/models with and without a catalog", "tests/console-api.test.ts"),
        ],
        depends_on=["T-1619"],
    ...
    Task(
        id="T-1622", slug="presets-and-first-mate", title="A preset library and the first mate orchestrator",
        epic="EP-16", sprint="SP-17", status="Done",
        ...
        assets=[
            ("src/defaults/agents/mate.md", "New", "The first mate: single point of contact, ship or scout, dispatch, supervise, escalate only real decisions, report in #bridge."),
            ("src/defaults/presets/researcher.md", "New", "Investigates against primary sources and leaves a report with evidence."),
            ("src/defaults/presets/reviewer.md", "New", "Reviews diffs for defects and missing tests; one line per finding."),
            ("src/defaults/presets/security-reviewer.md", "New", "Audits trust boundaries and secrets; ranks findings by exploitability."),
            ("src/defaults/presets/tech-writer.md", "New", "User-facing docs and changelog that the code does not contradict."),
            ("src/defaults/presets/sre.md", "New", "Builds, deploys, CI, incidents; every change reversible."),
            ("src/defaults/presets/debugger.md", "New", "Root cause by reproduction and bisection, with the failing test."),
            ("src/defaults/presets/test-engineer.md", "New", "Tests that fail when the code is wrong; flaky ones hardened."),
            ("src/defaults/presets/designer.md", "New", "Interfaces and flows specified to build, with every state and accessibility."),
            ("src/defaults/presets/release-manager.md", "New", "Cuts releases against the gates and the changelog; never force-pushes."),
            ("src/defaults/presets/data-analyst.md", "New", "Numbers with their query and their caveats."),
            ("src/daemon/presets.ts", "New", "Reads the library through the strict parser; a preset is named by its file."),
            ("src/daemon/runtime.ts", "Edited", "listPresets wired into the socket and console contexts."),
            ("src/shared/protocol.ts", "Edited", "presets_list and PresetInfo, additive."),
            ("src/shared/protocol-schemas.ts", "Edited", "presets_list validators; each preset validated as a create payload."),
            ("src/daemon/socket.ts", "Edited", "presets_list handler, worker-callable."),
            ("src/daemon/cli.ts", "Edited", "omp-agent presets and agent create <name> --preset <preset>."),
            ("src/daemon/console-api.ts", "Edited", "GET /api/presets."),
            ("src/extension/commands.ts", "Edited", "/preset: pick and name, or both up front; never spawns."),
            ("src/worker/toolbelt.ts", "Edited", "A presets_list tool and hiring guidance."),
            ("web/src/console/CreateAgentDialog.tsx", "Edited", "A Start from a preset picker that fills the form."),
            ("tests/default-peers.test.ts", "Edited", "Every preset parses and is a create payload; none is seeded; the mate's brief names the hiring calls."),
        ],
        steps=[
            "Write the presets and the mate as ordinary definitions with spawns: \"*\" and no model.",
            "Read the library through parsePeerDefinition and refuse a preset whose name disagrees with its file.",
            "Expose it as presets_list and copy a preset's fields unchanged apart from the name from the TUI, the CLI, the console, and the toolbelt.",
        ],
        acceptance=[
            "presets_list answers ten presets over the real socket, each accepted by agent_create under a new name.",
            "Seeding an empty store writes the staff and the mate and nothing from the presets.",
            "Each surface creates from a preset without spawning and refuses an unknown preset by name.",
        ],
        evidence=[
            ("Presets parse, are create payloads, and are never seeded; the mate's brief", "tests/default-peers.test.ts"),
            ("presets_list over the real socket and creation from a preset", "tests/daemon-main.test.ts"),
            ("omp-agent presets and agent create --preset", "tests/daemon-cli.test.ts"),
            ("/preset from the picker and with both arguments", "tests/extension.test.ts"),
            ("GET /api/presets with and without a library", "tests/console-api.test.ts"),
        ],
        depends_on=["T-1620"],
        out_of_scope=["Driving the mate through a real request on a live daemon: its brief is tested for shape, not behavior."],
    ),
    Task(
        id="T-1623", slug="channels-and-invites", title="Channels from every surface, membership with history, invite by mention",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="An operator or a peer can create a channel and assign peers to it from the TUI, the shell, the console, or the toolbelt; a join is persisted and hands a running peer the room's whole history; and an @mention in a room the peer is not in invites it the same way instead of delivering one message.",
        read_first=[
        ...
        assets=[
            ("src/daemon/supervisor.ts", "Edited", "An invite seam: a mention from outside the peer's rooms persists the room, resubscribes live, and delivers the backlog."),
            ("src/daemon/runtime.ts", "Edited", "setMembership, the single writer for non-console membership edits; membership frames refresh the peer index."),
            ("src/shared/protocol.ts", "Edited", "room_create, room_join, room_leave, additive."),
            ("src/shared/protocol-schemas.ts", "Edited", "Validators, with a room-id field check."),
            ("src/daemon/socket.ts", "Edited", "The three handlers; create and join worker-callable."),
            ("src/daemon/startup.ts", "Edited", "Usage lines."),
            ("src/daemon/cli.ts", "Edited", "rooms create, join, leave."),
            ("src/extension/commands.ts", "Edited", "/rooms create, join, leave."),
            ("src/extension/index.ts", "Edited", "The rooms dispatcher routes the new verbs."),
            ("src/worker/toolbelt.ts", "Edited", "room_create and room_join tools."),
            ("src/defaults/agents/mate.md", "Edited", "One channel per piece of work; brief the crew there."),
            ("tests/supervisor.test.ts", "Edited", "A mention from outside the peer's rooms invites it with the history."),
        ],
        steps=[
            "Persist membership through one runtime seam and apply it to a live peer only through the supervisor's resubscribe.",
            "Deliver after a live join: the subscription starts at cursor zero, so the next turn is the room's backlog.",
            "In the supervisor's post, route a mention from outside the peer's rooms through the invite seam, falling back to the single-message delivery when persisting fails.",
        ],
        acceptance=[
            "room_join on a running peer answers live and delivered, its definition lists the room, and its next turn carries the backlog; on a stopped peer it answers not live and the definition still lists the room.",
            "A mention in a room the peer left invites it back with the messages it missed.",
            "The TUI and CLI verbs report what the daemon did and refuse a missing agent.",
        ],
        evidence=[
            ("Invite by mention: persisted, live, with history", "tests/supervisor.test.ts"),
            ("room_create, room_join with history, room_leave, invite over the real socket", "tests/daemon-main.test.ts"),
            ("rooms create, join, leave from the CLI", "tests/daemon-cli.test.ts"),
            ("/rooms create, join, leave notices", "tests/extension.test.ts"),
        ],
        depends_on=["T-1622"],
        out_of_scope=["A console invite UI: posting there goes through the same supervisor path, so the behavior is already the same."],
    ),
    Task(
        id="T-1624", slug="status-reactions", title="Status reactions set by the daemon on delivery",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="Every message delivered to a peer carries that peer's seen mark, a message addressed to it carries an in-progress mark while its turn runs and a done or failed mark when the turn ends, all set by the daemon so a model cannot forget them; a peer's own reactions layer on top.",
        read_first=[
        ...
        assets=[
            ("src/daemon/supervisor.ts", "Edited", "Around the prompt: seen on everything delivered, in-progress then done or failed on what is addressed; a failed write is reported, never blocking."),
            ("src/worker/toolbelt.ts", "Edited", "chat_react guidance says what the daemon marks and when a peer's own reaction adds something."),
            ("tests/supervisor.test.ts", "Edited", "Marks during and after a turn; a throwing turn leaves the failed mark."),
            ("tests/toolbelt.test.ts", "Edited", "Existing reaction tests see the daemon's delivery mark beside the worker's own."),
        ],
        steps=[
            "In the supervisor's delivery, react on every delivered message before the prompt and on the addressed ones around it, from the same four emoji ADR-009 declared.",
            "Report a failed reaction write through onError and continue: status must never block a turn.",
        ],
        acceptance=[
            "During a turn the addressed message shows seen and in-progress; after it, seen and done.",
            "A turn that throws leaves seen and failed, and the error still surfaces.",
        ],
        evidence=[
            ("Reactions during and after a turn; a failed turn", "tests/supervisor.test.ts"),
            ("Worker reactions beside the daemon's marks", "tests/toolbelt.test.ts"),
        ],
        depends_on=["T-1623"],
        out_of_scope=["New emoji: the set stays the four ADR-009 chose, so the console renders them as status."],
    ),
    Task(
        id="T-1625", slug="setup-checklist", title="A setup checklist for operators and the OMP assistant",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="An operator runs one command and sees what a working install still needs, with the fix on every failing line; the TUI applies the two fixes it can, and a shipped skill lets the OMP assistant walk the operator through the same lines.",
        read_first=[
        ...
        assets=[
            ("src/shared/setup-report.ts", "New", "The checklist over a call function: daemon, models, default, crew, rooms; ✓/✗ lines with the fix."),
            ("src/extension/setup.ts", "New", "/setup: the checklist, then a model picker for the crew and a start for stopped peers."),
            ("src/extension/commands.ts", "Edited", "guard is exported for the setup command."),
            ("src/extension/index.ts", "Edited", "/setup registered."),
            ("src/daemon/cli.ts", "Edited", "omp-agent setup prints the same checklist."),
            ("src/daemon/startup.ts", "Edited", "Usage line."),
            ("skills/oh-my-agent-setup/SKILL.md", "New", "What each ✗ line means and the fix, a smoke request in #bridge, and what not to do."),
            ("tests/extension.test.ts", "Edited", "Daemon down; a ready install; the model pick and the crew start."),
            ("tests/daemon-cli.test.ts", "Edited", "The checklist in text and JSON."),
            ("tests/skills.test.ts", "Edited", "The setup skill is packaged and loads."),
        ],
        steps=[
            "Collect the checklist over the existing methods only: status, models_list, rooms_list.",
            "Offer a fix only when its precondition holds, and write a model only to crew peers still on the default.",
            "Write the skill against the checklist's own lines so the assistant and the command never disagree.",
        ],
        acceptance=[
            "A down daemon yields one ✗ line with the restart command and no prompts.",
            "A complete install yields only ✓ lines ending with the mate's address.",
            "An unroutable default offers the catalog and writes the pick to the crew peers on the default; stopped crew are offered a start and spawned on confirmation.",
        ],
        evidence=[
            ("Daemon down, ready install, model pick, crew start", "tests/extension.test.ts"),
            ("omp-agent setup in text and JSON", "tests/daemon-cli.test.ts"),
            ("The skill is packaged", "tests/skills.test.ts"),
        ],
        depends_on=["T-1624"],
        out_of_scope=["Writing models.yml for the operator: a provider entry needs credentials the daemon must not handle from a prompt."],
    ),
    Task(
        id="T-1626", slug="heartbeat", title="A heartbeat so peers keep working without an orchestrator",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="A peer can declare a standing interval and receive a turn on it while running — read your rooms and plans, continue unfinished work, answer idle if nothing is pending — queued behind any turn in flight, listed and switchable beside the schedules, so a crew carries on with nobody waking it.",
        read_first=[
        ...
        assets=[
            ("src/shared/duration.ts", "New", "Parses 90s, 30m, 2h, 1d into milliseconds; dependency-free."),
            ("src/shared/agent-definition.ts", "Edited", "The heartbeat key: every (a duration of at least 10s) and an optional prompt; INVALID_HEARTBEAT."),
            ("src/shared/protocol.ts", "Edited", "PresetInfo carries heartbeat."),
            ("src/shared/protocol-schemas.ts", "Edited", "The same check on the wire."),
            ("src/daemon/supervisor.ts", "Edited", "nudge: prompt a running peer through the per-peer queue; delivery and nudges share one serializer."),
            ("src/daemon/runtime.ts", "Edited", "armHeartbeat: a one-shot re-armed after each firing, listed as <peer>:heartbeat, disarmable and persisted like a schedule."),
            ("src/daemon/presets.ts", "Edited", "A preset's heartbeat travels with it."),
            ("src/daemon/peer-store.ts", "Edited", "heartbeat is a stored definition key."),
            ("src/daemon/console-api.ts", "Edited", "heartbeat is a wire definition field."),
            ("src/defaults/agents/mate.md", "Edited", "every: 30m."),
            ("src/defaults/agents/staff-pm.md", "Edited", "every: 30m."),
            ("src/defaults/agents/staff-backend.md", "Edited", "every: 30m."),
            ("src/defaults/agents/staff-frontend.md", "Edited", "every: 30m."),
            ("src/defaults/agents/staff-qa.md", "Edited", "every: 30m."),
            ("tests/agent-definition.test.ts", "Edited", "A heartbeat parses; a short, malformed, or over-keyed one is refused."),
            ("tests/supervisor.test.ts", "Edited", "A nudge waits behind a turn in flight and skips a peer that is not running."),
            ("tests/daemon-main.test.ts", "Edited", "Listed, fires into the worker, disarms, and stays disarmed across a restart."),
            ("tests/default-peers.test.ts", "Edited", "The crew and every preset declare one."),
        ],
        steps=[
            "Parse and validate the key once, in the definition parser, and mirror the check in the wire validator with a shared duration parser.",
            "Deliver through the supervisor's serializer so a heartbeat never lands mid-turn, and only to a running peer.",
            "Arm it as a re-arming one-shot under the schedule id, so the existing list and arm surfaces cover it.",
        ],
        acceptance=[
            "A peer with heartbeat every 10s receives the prompt within 15s of boot and its schedule row shows the next fire.",
            "Disarming clears the next fire and survives a restart.",
            "A nudge issued during a turn runs after it; a parked peer is not nudged.",
        ],
        evidence=[
            ("Heartbeat parses and is refused when malformed", "tests/agent-definition.test.ts"),
            ("Nudge serialization and gating", "tests/supervisor.test.ts"),
            ("Listed, fired, disarmed, persisted", "tests/daemon-main.test.ts"),
            ("Crew and presets declare it", "tests/default-peers.test.ts"),
        ],
        depends_on=["T-1625"],
        out_of_scope=["A heartbeat that resumes a quota-parked peer: the block is the registry's decision, and delivery respects it the same way."],
    ),
    Task(
        id="T-1627", slug="console-markdown-mermaid", title="Message and plan bodies render as Markdown with mermaid diagrams",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="What an agent writes in a room reads the way it was meant: GitHub-flavored Markdown for messages and plans, fenced code kept as code with diff lines tinted, and mermaid fences drawn as diagrams in the console's own palette, with a diagram that fails to parse shown as its source rather than a blank.",
        read_first=[
        ...
        assets=[
            ("web/src/console/Markdown.tsx", "New", "react-markdown with remark-gfm; code, diff, mermaid (lazy, theme-aware), mentions in prose; HTML stays text."),
            ("web/src/console/Message.tsx", "Edited", "MessageBody delegates to Markdown; the hand-rolled line parser is gone."),
            ("web/vite.config.ts", "Edited", "inlineDynamicImports, so mermaid's lazily loaded modules live inside the one served app.js."),
            ("web/package.json", "Edited", "react-markdown, remark-gfm, mermaid."),
            ("web/bun.lock", "Edited", "Locked."),
            ("src/console/app.js", "Edited", "Rebuilt."),
            ("src/console/style.css", "Edited", "Rebuilt."),
            ("tests/console-client.test.ts", "Edited", "A body with a heading, table, task list, inline code, HTML, a diff fence, and a mermaid fence renders each as intended."),
        ],
        steps=[
            "Replace the line-based renderer with react-markdown and GFM, keeping the code-block styling and diff tint as custom components.",
            "Render a mermaid fence through mermaid.render on first sight, in the console's light or dark theme, falling back to the source on error.",
            "Keep the bundle a single file, since the daemon serves an allowlist, by inlining dynamic imports.",
        ],
        acceptance=[
            "A message with a heading, a table with bold, a checked task, inline code, raw HTML, a diff fence, and a mermaid fence renders the heading, the table, the checkbox, the code, the HTML as text, the tinted diff line, and an SVG with the diagram's labels.",
            "The browser suite stays green with no client-side errors.",
        ],
        evidence=[
            ("Markdown and mermaid rendering in the browser", "tests/console-client.test.ts"),
        ],
        depends_on=["T-1626"],
        out_of_scope=["Splitting mermaid into a separately served chunk: the daemon's static allowlist is deliberate, and the bundle stays one file at the cost of size."],
    ),
    Task(
        id="T-1628", slug="console-schedules-profile", title="Schedules and a display profile in the console",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="An operator sees every schedule and heartbeat in the console with its next fire, pauses or resumes each, adds a cron schedule to an agent, and sets a display name and avatar for themselves and for each agent that every console then shows.",
        read_first=[
        ...
        assets=[
            ("src/daemon/profile.ts", "New", "The display profile store: validated personas for the operator and per agent, written atomically under the state dir."),
            ("src/daemon/console-api.ts", "Edited", "GET /api/schedules, PATCH /api/schedules/:id, GET/PUT /api/profile, and a profile frame."),
            ("src/daemon/runtime.ts", "Edited", "Hands the console its schedule map, armSchedule, and a profile store."),
            ("web/src/console/profile.ts", "New", "The profile context and personaFor: a wire author to its display name and avatar."),
            ("web/src/console/ProfileDialog.tsx", "New", "Edit the operator's and each agent's display name and avatar; saved through PUT /api/profile."),
            ("web/src/console/SchedulesTab.tsx", "New", "List with next fire and switch; a form that appends a cron schedule to an agent's definition."),
            ("web/src/console/AgentPanel.tsx", "Edited", "A Schedules tab."),
            ("web/src/console/ConsoleShell.tsx", "Edited", "Provides the profile context and mounts the dialog."),
            ("web/src/console/WorkspaceToolbar.tsx", "Edited", "A Profile and avatars button."),
            ("web/src/console/Message.tsx", "Edited", "Author and avatar drawn from the profile."),
            ("web/src/console/useConsole.ts", "Edited", "Loads the profile on connect and on its frame."),
            ("web/src/console/Storybook.tsx", "Edited", "Fixtures for both routes."),
            ("web/src/lib/types.ts", "Edited", "The profile frame."),
            ("src/console/app.js", "Edited", "Rebuilt."),
            ("src/console/style.css", "Edited", "Rebuilt."),
            ("tests/console-api.test.ts", "Edited", "Schedules list and flip; profile validation, persistence, and frame."),
            ("tests/console-client.test.ts", "Edited", "The dialog renames the operator and an agent and survives a reload; the tab pauses a heartbeat."),
        ],
        steps=[
            "Expose the daemon's schedule map and arm switch to the console over two routes, gated like membership.",
            "Keep the profile cosmetic: a validated JSON file the console reads, never an author on the wire.",
            "Resolve every drawn author through one context so the transcript, threads, and the dialog agree.",
        ],
        acceptance=[
            "The schedules tab lists a heartbeat and pausing it flips the daemon's record.",
            "Saving a display name renames the operator and the agent in the transcript and survives a reload.",
            "A field over its length, an unknown field, or a bad agent name is refused with a 400 and nothing is written.",
        ],
        evidence=[
            ("Schedule routes and profile validation, persistence, and frame", "tests/console-api.test.ts"),
            ("The profile dialog and the schedules tab in the browser", "tests/console-client.test.ts"),
        ],
        depends_on=["T-1627"],
        out_of_scope=["Editing an existing schedule's cron in place: the definition editor already carries the schedules field for that."],
    ),
    Task(
        id="T-1629", slug="lavish-artifacts", title="Lavish Editor for reviewable HTML artifacts",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="A peer can hand the operator a plan, comparison, or report as a page: it writes HTML, opens it with Lavish Editor headless, and polls for feedback; the operator finds every such session in the console's Artifacts view and opens the review from there. On the way, definitions that select skills start at all: the runtime now hands the materializer the package's skill roots.",
        read_first=[
        ...
        assets=[
            ("src/daemon/skill-roots.ts", "New", "The package's skills directory as name to root; the map the materializer was never given."),
            ("src/daemon/artifacts.ts", "New", "Lavish sessions from its state.json, and a bounded resume through npx or bunx."),
            ("src/daemon/runtime.ts", "Edited", "Hands packageSkillRoots to every materialization."),
            ("src/daemon/materializer.ts", "Edited", "Workers get LAVISH_AXI_NO_OPEN and the operator's LAVISH_AXI_STATE_DIR."),
            ("src/daemon/console-api.ts", "Edited", "GET and POST /api/artifacts."),
            ("skills/lavish/SKILL.md", "New", "A stub that sends the agent to the lavish-axi CLI for current guidance, plus how it fits this daemon."),
            ("src/defaults/agents/mate.md", "Edited", "Reach for an artifact when the operator should see rather than read; skills: lavish."),
            ("src/defaults/agents/staff-pm.md", "Edited", "skills: lavish."),
            ("src/defaults/agents/staff-backend.md", "Edited", "skills: lavish."),
            ("src/defaults/agents/staff-frontend.md", "Edited", "skills: lavish."),
            ("src/defaults/agents/staff-qa.md", "Edited", "skills: lavish."),
            ("web/src/console/ArtifactsView.tsx", "New", "The Artifacts view: sessions with status and queued prompts, Open review resumes and opens."),
            ("web/src/console/ConsoleShell.tsx", "Edited", "An Artifacts tab."),
            ("web/src/console/Storybook.tsx", "Edited", "Fixtures and a story for the view."),
            ("storybook/console/stories.js", "Edited", "The page-artifacts story."),
            ("src/console/app.js", "Edited", "Rebuilt."),
            ("src/console/style.css", "Edited", "Rebuilt."),
            ("tests/console-api.test.ts", "Edited", "Listing from a fixture state file; resume of a known session; refusals."),
            ("tests/materializer.test.ts", "Edited", "The worker env runs Lavish headless in the operator's state dir."),
            ("tests/skills.test.ts", "Edited", "The lavish skill is packaged."),
            ("tests/default-peers.test.ts", "Edited", "The crew and every preset declare it."),
        ],
        steps=[
            "Ship a stub skill that defers to the CLI, as upstream does, so it cannot go stale, and declare it on every default peer and preset.",
            "Give the materializer the package's skill roots, and give workers a headless Lavish pointed at the operator's state dir.",
            "List sessions from Lavish's own state and resume one on request; never open a file Lavish does not already hold.",
        ],
        acceptance=[
            "A definition with skills: [lavish] materializes with the skill under its agent dir.",
            "The Artifacts view lists sessions newest first and Open review resumes the chosen one.",
            "A worker's env carries LAVISH_AXI_NO_OPEN=1 and a state dir outside its synthetic HOME.",
        ],
        evidence=[
            ("Artifact routes over a fixture state file", "tests/console-api.test.ts"),
            ("Worker env for Lavish", "tests/materializer.test.ts"),
            ("The skill is packaged and loads", "tests/skills.test.ts"),
            ("Crew and presets declare it", "tests/default-peers.test.ts"),
        ],
        depends_on=["T-1628"],
        out_of_scope=["Bundling lavish-axi as a dependency: the package ships no runtime dependencies, and Lavish is an AXI meant to run through npx on demand."],
    ),
    Task(
        id="T-1630", slug="console-lazy-chunks", title="A small console bundle: lazy chunks, gzip, immutable caching",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="The console's first load is under a megabyte again: mermaid and its diagram packs are content-hashed chunks fetched only when a diagram renders, served immutable and gzipped, with the daemon still serving an allowlist of shapes and remote mode still gating every asset behind a ticket.",
        read_first=[
        ...
        assets=[
            ("web/vite.config.ts", "Edited", "Hashed chunk names and a runtime URL minter for dynamic imports."),
            ("web/src/main.tsx", "Edited", "A default minter for the dev server."),
            ("src/daemon/console-api.ts", "Edited", "Chunk shape in the allowlist, immutable caching, gzip with an mtime-keyed cache, the injected minter, and a reusable prefix-bound chunk pass in remote mode."),
            ("src/console/app.js", "Edited", "Rebuilt, under a megabyte."),
            ("src/console/style.css", "Edited", "Rebuilt."),
            ("tests/daemon-console-mount.test.ts", "Edited", "A chunk exists and app.js is small; a chunk is served immutable and gzipped with the token-minted URL; a chunk never built is 404."),
            ("tests/remote-exposure.test.ts", "Edited", "The remote shell carries a chunk pass that serves a chunk twice and nothing else."),
        ],
        steps=[
            "Let the build emit chunks under one recognizable shape and mint their URLs at runtime through an injected function.",
            "Serve that shape with immutable caching and gzip, and keep everything else under the fixed allowlist.",
            "In remote mode, mint one reusable pass bound to the chunk prefix into the shell, since one-time tickets cannot cover a chunk fetched an hour later.",
        ],
        acceptance=[
            "app.js is under 1.5 MB and a mermaid chunk exists beside it.",
            "A chunk answers 200 with immutable caching, gzipped when accepted, 401 without a credential, and a chunk that was never built is 404.",
            "In remote mode the pass serves a chunk repeatedly and is refused for app.js.",
        ],
        evidence=[
            ("Chunk serving on loopback", "tests/daemon-console-mount.test.ts"),
            ("The chunk pass in remote mode", "tests/remote-exposure.test.ts"),
            ("Mermaid still renders from a lazy chunk", "tests/console-client.test.ts"),
        ],
        depends_on=["T-1629"],
    ...
    Task(
        ...
        assets=[
            ("web/src/lib/themes.ts", "New", "Six curated sidebar themes after Slack's; each sets only the --ws-* chrome variables."),
            ("web/src/lib/markdown-format.ts", "New", "Pure helpers that wrap or insert Markdown at a textarea selection for the composer's formatting row."),
            ("web/src/lib/panel-size.ts", "New", "Clamps and persists the resizable sidebar and thread widths, and decides when the thread docks."),
            ("web/src/console/EmojiPicker.tsx", "New", "The full emoji picker, loaded as a lazy chunk with bundled data; used for reactions and the composer."),
            ("web/src/console/definition/model.ts", "New", "Builds the definition PATCH from a draft: only changed fields, with the account ceiling carried forward and never edited."),
            ("web/src/console/definition/fields.tsx", "New", "Shared form controls for the agent settings sections."),
            ("web/src/console/definition/identity-sections.tsx", "New", "Soul with Markdown preview, profile, model, and rooms and hierarchy sections."),
            ("web/src/console/definition/policy-sections.tsx", "New", "Wake and autonomy, schedules and automations, and sandbox and tools sections."),
            ("web/src/console/definition/AvatarEditor.tsx", "New", "Emoji or image avatar editor that downscales uploads to 256 pixels before saving."),
            ("tests/markdown-format.test.ts", "New", "Wrapping, toggling, and insertion at a selection."),
            ("tests/panel-size.test.ts", "New", "Width bounds, percentage caps, the channel minimum, the dock threshold, and storage failures."),
            ("tests/profile.test.ts", "New", "Grapheme avatars, image avatars at and over the size cap, refused mime types and malformed base64."),
            ("tests/definition-patch.test.ts", "New", "Only changed fields are sent, and the account ceiling is carried forward and never changed."),
            ("web/src/lib/theme.ts", "Edited", "Applies the curated theme's chrome variables; a stored id from the old catalog falls back to Aubergine."),
            ("web/src/index.css", "Edited", "Slack light and dark content tokens that keep AAA text contrast, chrome gradients, and component classes."),
            ("web/src/console/ThemeSelector.tsx", "Edited", "Theme cards drawn as miniature workspaces beside the light, dark, and system modes."),
            ("web/src/console/WorkspaceToolbar.tsx", "Edited", "Gradient toolbar with centered search, the labeled icon rail, and avatar tiles that draw image avatars."),
            ("web/src/console/ChannelRail.tsx", "Edited", "Gradient sidebar with a conversation filter, unread badges, presence dots, and a resize handle."),
            ("web/src/console/ConsoleShell.tsx", "Edited", "Channel header with member stack, underline tabs with the working-directory chip, fluid rows, and the search palette."),
            ("web/src/console/Transcript.tsx", "Edited", "Date pills between days."),
            ("web/src/console/Message.tsx", "Edited", "Hover action toolbar, reaction pills with the full picker, thread summary rows, and image avatars."),
            ("web/src/console/ThreadPanel.tsx", "Edited", "Resizable docked panel that becomes an overlay when the channel column would drop under 480 pixels."),
            ("web/src/console/Composer.tsx", "Edited", "Bordered box with a Markdown formatting row, emoji and mention actions, and the green send button."),
            ("web/src/console/Markdown.tsx", "Edited", "Slack code, list, link, and mention styling."),
            ("web/src/console/AgentPanel.tsx", "Edited", "Members, Operations, and Schedules; the account-ceiling tab is gone and each agent opens full settings."),
            ("web/src/console/DefinitionDialog.tsx", "Edited", "Sectioned agent settings over the existing definition PATCH, keeping the raw JSON editor as Advanced."),
            ("web/src/console/SchedulesTab.tsx", "Edited", "Scrolls inside the sheet, never clips its buttons, and adds a schedule without replacing the others."),
            ("web/src/console/ProfileDialog.tsx", "Edited", "Emoji or image avatars for the operator and every agent; sends only changed personas."),
            ("web/src/console/profile.ts", "Edited", "Recognizes image avatars."),
            ("web/src/console/CreateAgentDialog.tsx", "Edited", "Avatar at creation; no account ceiling field."),
            ("web/src/console/ChangesView.tsx", "Edited", "Fluid grid and a diff viewer that scrolls long lines inside itself."),
            ("web/src/console/PlansView.tsx", "Edited", "Fluid card grid."),
            ("web/src/console/Storybook.tsx", "Edited", "Stories follow the new shell, with richer definition and profile fixtures."),
            ("web/package.json", "Edited", "emoji-mart and its data, pinned."),
            ("web/vite.config.ts", "Edited", "The HTML rewrite runs only for builds, so the dev server loads the source entry."),
            ("web/DESIGN.md", "Edited", "The Slack visual system and curated themes."),
            ("src/worker/toolbelt.ts", "Edited", "Agents may react with any single emoji instead of four."),
            ("src/daemon/profile.ts", "Edited", "Avatars may be small data-URL images as well as up to four characters."),
            ("tests/toolbelt.test.ts", "Edited", "Any single emoji accepted; letters, pairs, and shortcodes refused."),
            ("tests/console-client.test.ts", "Edited", "Appearance picks a curated theme; the removed ceiling tab's test is gone; the definition editor opens on the soul."),
            ("src/console/app.js", "Edited", "Regenerated from web/."),
            ("src/console/style.css", "Edited", "Regenerated from web/."),
        ],
        steps=[
            "Replace the generated palettes with Slack content tokens that keep AAA contrast and curated chrome themes.",
            "Rebuild the chrome, conversation, and overlays against those tokens without moving any id the browser suite reads.",
            "Make the layout fluid: resizable sidebar and thread, a thread that docks only when the channel keeps its width, and views that scroll inside themselves.",
            "Give the browser the rest of an agent: sectioned settings over the existing PATCH, any emoji for reactions from people and agents, and image avatars; drop the account ceiling from the console.",
        ],
        acceptance=[
            "The console browser suite passes against the rebuilt console, including the AAA contrast, reduced-motion, keyboard, and 320/390-pixel journeys.",
            "No page overflows horizontally from 320 to 2560 pixels wide in any story, and the sidebar and thread widths clamp and persist.",
            "An agent can react with any single emoji and is refused for letters, pairs, and shortcodes.",
            "An avatar is up to four characters or a PNG, JPEG, WebP, or GIF data URL of at most 200 KB, and anything else is refused.",
            "A definition edit sends only changed fields and never changes an account ceiling.",
        ],
        evidence=[
            ("72 of 72 browser tests pass against the rebuilt console on 2026-09-14", "tests/console-client.test.ts"),
            ("Width clamps and persistence", "tests/panel-size.test.ts"),
            ("Image avatar validation", "tests/profile.test.ts"),
            ("Changed-field patches that carry the ceiling forward", "tests/definition-patch.test.ts"),
            ("Any single emoji for agents", "tests/toolbelt.test.ts"),
            ("Formatting-row insertion", "tests/markdown-format.test.ts"),
        ],
        depends_on=["T-1631"],
        out_of_scope=["Account ceilings: the daemon, the CLI bump, and quota parking are unchanged; the console no longer edits them. A webfont: the console keeps system fonts. The server-rendered console in web-next/."],
    ),
    Task(
        id="T-1633", slug="blobatar-default-avatars", title="Default avatars are blobatars",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="Every author with no stored avatar draws a blobatar generated from their wire name in the console, the public demo, the homepage transcript, and the TUI manager; a ⏳ mid-turn takes the thinking pose; custom glyphs and images still override.",
        read_first=[
        ...
        assets=[
            ("src/shared/avatar.ts", "New", "resolveAvatar and thinkingActors; empty stored avatar is a blobatar of the wire name."),
            ("src/shared/tui-face.ts", "New", "TUI disc from the blobatar body fill."),
            ("web/src/components/ui/blobatar.tsx", "New", "shadcn Avatar composition with a blobatar fallback."),
            ("web/src/console/thinking.tsx", "New", "Actors holding ⏳ on loaded messages."),
            ("tests/avatar.test.ts", "New", "Empty is blobatar; glyph and image win; same name same SVG; TUI disc."),
            ("scripts/capture-console-assets.ts", "New", "Storybook screenshots for docs/assets and the website."),
            ("src/extension/manager.ts", "Edited", "Each agent row starts with a blobatar disc."),
            ("src/daemon/profile.ts", "Edited", "Empty avatar means consoles draw a blobatar from the wire name."),
            ("package.json", "Edited", "blobatar is the plugin's only runtime dependency."),
            ("bun.lock", "Edited", "Locked blobatar."),
            ("tsconfig.json", "Edited", "Exclude leftover web-next/ from tsc."),
            ("biome.json", "Edited", "Ignore leftover web-next/."),
            ("tests/build-hygiene.test.ts", "Edited", "blobatar is the only allowed runtime dependency."),
            ("tests/extension.test.ts", "Edited", "Manager rows include the disc."),
            ("tests/console-client.test.ts", "Edited", "Empty profile draws a blobatar; a glyph still wins."),
            ("web/package.json", "Edited", "blobatar and @blobatar/react."),
            ("web/bun.lock", "Edited", "Locked."),
            ("web/components.json", "Edited", "The @blobatar shadcn registry."),
            ("web/src/App.tsx", "Edited", "Loads blobatar/motion.css for the thinking pose."),
            ("web/src/console/profile.ts", "Edited", "personaFor no longer invents initials; resolveAvatar is the drawing."),
            ("web/src/console/WorkspaceToolbar.tsx", "Edited", "AvatarTile draws image, glyph, or blobatar; thinking pose while ⏳."),
            ("web/src/console/Message.tsx", "Edited", "Message avatars use AvatarTile."),
            ("web/src/console/ConsoleShell.tsx", "Edited", "ThinkingProvider from loaded messages."),
            ("web/src/console/Storybook.tsx", "Edited", "Empty profile fixtures; researcher holds ⏳."),
            ("web/src/console/ProfileDialog.tsx", "Edited", "Empty field is a blobatar of the wire name."),
            ("web/src/console/CreateAgentDialog.tsx", "Edited", "Preview is a blobatar of the draft name."),
            ("web/src/console/definition/AvatarEditor.tsx", "Edited", "Empty preview is a blobatar."),
            ("web/src/console/definition/identity-sections.tsx", "Edited", "Passes the wire name into the editor."),
            ("website/package.json", "Edited", "blobatar and @blobatar/react."),
            ("website/bun.lock", "Edited", "Locked."),
            ("website/src/mock/fixtures/agents.ts", "Edited", "Demo seed has names, no emoji avatars."),
            ("website/src/mock/fixtures/index.ts", "Edited", "STATE_VERSION 3 so old demo storage reseeds."),
            ("website/src/components/home/sections/Transcript.tsx", "Edited", "Homepage faces are blobatars."),
            ("website/src/app/(home)/story.css", "Edited", "Avatar tile is the blobatar, not a letter plate."),
            ("website/public/home/console.png", "Edited", "Storybook capture of the populated console."),
            ("website/public/home/collaboration.png", "Edited", "Storybook capture of the open thread."),
            ("docs/assets/console.png", "Edited", "Storybook capture of the populated console."),
            ("docs/assets/collaboration.png", "Edited", "Storybook capture of the open thread."),
            ("docs/guide/console.md", "Edited", "Default avatar is a blobatar."),
            ("docs/web-console.md", "Edited", "Default avatar is a blobatar."),
            ("web/DESIGN.md", "Edited", "Message avatars default to a blobatar."),
            ("CHANGELOG.md", "Edited", "Unreleased: default avatars are blobatars."),
            ("src/console/app.js", "Edited", "Regenerated from web/."),
            ("src/console/style.css", "Edited", "Regenerated from web/."),
        ],
        steps=[
            "Resolve empty avatars to a blobatar of the wire name; keep stored glyphs and images.",
            "Paint that face in the console, the demo, the homepage transcript, and the TUI manager.",
            "Hold the thinking pose while the actor has ⏳ on a loaded message.",
            "Regenerate the conversation and collaboration screenshots from the storybook.",
        ],
        acceptance=[
            "An empty profile draws a blobatar from the wire author, proven by reverting the fallback line.",
            "A stored glyph or image still draws instead of a blobatar.",
            "A ⏳ reaction puts that actor's default blobatar in the thinking pose.",
            "The TUI manager row includes a disc of the blobatar body color.",
        ],
        evidence=[
            ("Empty stored avatar is a blobatar of the wire author", "tests/avatar.test.ts"),
            ("Browser: default face is a blobatar; a glyph still wins", "tests/console-client.test.ts"),
            ("Manager rows include the disc", "tests/extension.test.ts"),
        ],
        depends_on=["T-1632"],
        out_of_scope=["Wiping stored custom avatars. Gaze. A self-hosted blobatar HTTP endpoint."],
    ),
    Task(
        ...
        assets=[
            ...
            ("website/src/components/demo/DemoBanner.tsx", "New", "Demo notice with reset."),
            ...
            ("website/src/mock/demoToken.ts", "New", "The operator token the mock accepts; written into the session before the console mounts."),
        ...
        steps=[
            ...
            "Mount the real console client-only with the demo token already in the session; patch fetch, the events WebSocket, and the upload XHR; seed fixtures and persist state; answer every route in console-api.ts.",
        ...
        acceptance=[
            "Opening /console lands on the console shell with the seeded rooms; a request without the demo token is refused with 401.",
        ...
        evidence=[
            ("Every view and dialog driven in Chrome at 1440 and 390 wide with zero uncaught errors, on 2026-09-14", "website/src/components/demo/ConsoleApp.tsx"),
            ...
            ("next build lists /, /console, /console/review/[id]; tsc clean in website; root lint and typecheck clean with website excluded", "website/package.json"),
        ],
        out_of_scope=["Lighthouse and frame-rate measurement on real GPUs. The two agent-sheet layout defects the demo first worked around in its own CSS are fixed in web/ by T-1632."],
    ),
]
...
TASKS += [
    ...
    Task(
        ...
        assets=[
            ("src/daemon/console-api.ts", "Edited", "Static cookie minted on token-bearing document loads and accepted for static files only; bare app.js; no token stamping."),
            ("web/src/lib/api.ts", "Edited", "ApiError with code and status; non-JSON and empty bodies; 401 is auth-required in both modes."),
            ("web/src/lib/token.ts", "New", "readToken, moved out so api.ts is unit-testable."),
            ("web/src/lib/attachments.ts", "Edited", "Upload and delete 401s go through the shared revoke path."),
            ("web/src/main.tsx", "Edited", "Drops the __omaAsset helper."),
            ("web/vite.config.ts", "Edited", "No renderBuiltUrl; the dev proxy finds the daemon and forwards the WebSocket with a matching Origin."),
            ("web/src/console/useConsole.ts", "Edited", "A shared revoke handler for refused tokens."),
            ("web/src/console/ConsoleShell.tsx", "Edited", "Passes the revoke handler."),
            ("web/src/console/AuthScreen.tsx", "Edited", "Wording for a refused loopback token."),
            ("tests/console-client.test.ts", "Edited", "Loopback static requests go through the real daemon; reload, Mermaid, refused-token, and remote Mermaid tests."),
            ("tests/console-api-client.test.ts", "New", "Unit tests for api() status, code, and body handling."),
            ("tests/daemon-console-mount.test.ts", "Edited", "Static cookie scope and HMAC tests."),
            ("tests/remote-exposure.test.ts", "Edited", "Remote asset-pass cookie tests."),
            ("docs/web-console.md", "Edited", "Static cookie auth."),
            ("docs/guide/console.md", "Edited", "Reload and dev-proxy behaviour."),
            ("docs/remote-exposure.md", "Edited", "Asset pass travels in a Secure cookie."),
        ],
        steps=[
            "Replace token stamping with a static-only cookie on loopback and in remote mode.",
            "Serve the browser suite's console through the daemon's static path.",
            "Make api() errors carry the daemon's code and status.",
        ],
        acceptance=[
            "A reload of a loopback console renders the console, and no URL carries the token.",
            "A Mermaid fence renders on a real daemon in loopback and remote mode.",
            "The static cookie never authenticates /api or the WebSocket.",
        ],
        evidence=[
            ("Reload, Mermaid, and refused-token browser tests through the daemon", "tests/console-client.test.ts"),
            ("Cookie scope tests", "tests/daemon-console-mount.test.ts"),
        ],
    ),
    Task(
        id="T-1802", slug="console-newest-messages", title="Rooms open on their newest messages and chats stop refetch storms",
        epic="EP-18", sprint="SP-19", status="Done",
        goal="A room past 500 messages opens on its newest page and pages back on demand. Chat token deltas no longer refetch every open console, unread badges cover rooms never opened, and a failed bootstrap recovers when the socket opens.",
        read_first=[
        ...
        assets=[
            ("src/rooms/store.ts", "Edited", "newest and beforeId paging with older thread roots included."),
            ("src/daemon/console-api.ts", "Edited", "newest and beforeId on the messages route."),
            ("web/src/console/useConsole.ts", "Edited", "Newest page on open, load older, per-chat throttled refetch, unread seeding, bootstrap recovery, restart hint, test hooks behind a flag."),
            ("web/src/console/Transcript.tsx", "Edited", "Load older control and scroll anchoring."),
            ("web/src/console/ConsoleShell.tsx", "Edited", "Transcript paging props; chat refetch scoped to the viewed chat."),
            ("tests/rooms.test.ts", "Edited", "Store paging tests."),
            ("tests/console-api.test.ts", "Edited", "Messages route paging test."),
            ("tests/console-client.test.ts", "Edited", "Large room, delta refetch count, unread, bootstrap, restart hint, and hook tests."),
            ("docs/web-console.md", "Edited", "Messages route query parameters."),
        ],
        steps=[
            "Add newest and beforeId to the store and route.",
            "Open rooms on the newest page with a load-older control.",
            "Scope and throttle chat refetches; fix unread and bootstrap recovery.",
        ],
        acceptance=[
            "A room with 520 messages shows its newest message on open and loads older history on demand.",
            "A streaming chat does not refetch per token in consoles that are not viewing it.",
            "A never-opened room gets an unread badge after a reconnect.",
        ],
        evidence=[
            ("Large-room and refetch browser tests", "tests/console-client.test.ts"),
            ("Store paging tests", "tests/rooms.test.ts"),
        ],
    ),
    Task(
        id="T-1803", slug="daemon-review-hardening", title="Remote policy gate and daemon hardening",
        epic="EP-18", sprint="SP-19", status="Done",
        goal="Remote requests without full control can change only an agent's name, description, model, thinking level, and rooms. Profile saves, uploads, web chats, workspace inspection, setup reporting, directory listing, plan routes, and failed boots behave correctly under the edge cases the review found, and the TUI and CLI agree.",
        read_first=[
        ...
        assets=[
            ("src/daemon/console-api.ts", "Edited", "Allowlist of remote-editable agent fields; request body cap."),
            ("src/daemon/profile.ts", "Edited", "Serialized updates with unique temp files."),
            ("src/daemon/web-attachments.ts", "Edited", "Per-file size cap and count limits with 413 and 429."),
            ("src/daemon/web-chats.ts", "Edited", "Private storage directory, OMP in its own process group, files removed on close."),
            ("src/daemon/workspace-changes.ts", "Edited", "Submodule filters ignored, non-UTF-8 names tolerated, status truncation."),
            ("src/daemon/web-files.ts", "Edited", "Sort before capping and report truncation."),
            ("src/daemon/web-routes.ts", "Edited", "Method checked before the body."),
            ("src/daemon/runtime.ts", "Edited", "Console URL ordering; workers stopped when a later boot step fails."),
            ("src/daemon/cli.ts", "Edited", "Unknown leading flags are errors."),
            ("src/extension/commands.ts", "Edited", "/logs daemon, /kill keep-children, /preset argument order, /edit."),
            ("src/extension/index.ts", "Edited", "Registers /edit."),
            ("src/shared/setup-report.ts", "Edited", "Parked crew and drift direction."),
            ("tests/daemon-cli.test.ts", "Edited", "Unknown flag test."),
            ("tests/daemon-runtime-composition.test.ts", "New", "Boot unwind, invalid console port, shutdown during spawn, and wired web routes through bootDaemon."),
            ("tests/extension.test.ts", "Edited", "TUI command alignment tests."),
            ("tests/profile.test.ts", "Edited", "Concurrent update test."),
            ("tests/setup-report.test.ts", "New", "Parked crew and drift direction."),
            ("tests/web-attachments.test.ts", "Edited", "Size and count limit tests."),
            ("tests/web-chats-lifecycle.test.ts", "New", "Storage directory, process group, and close cleanup with a stand-in CLI."),
            ("tests/web-files.test.ts", "New", "Listing order, cap, and attachmentReferences."),
            ("tests/web-workspace-security.test.ts", "Edited", "Remote policy field gate tests."),
            ("tests/workspace-changes.test.ts", "Edited", "Submodule filter, non-UTF-8 name, and truncation tests."),
            ("docs/develop/modules.md", "Edited", "Every source module and test suite."),
            ("docs/guide/cli.md", "Edited", "models verb and TUI counterparts."),
            ("docs/guide/console.md", "Edited", "What remote operators may edit without full control."),
            ("docs/web-console.md", "Edited", "Agent route gate and upload limits."),
        ],
        steps=[
            "Gate policy-bearing agent fields behind remote full control with an allowlist.",
            "Fix each daemon edge case with a test that fails without the fix.",
            "Align the TUI with the CLI and refresh the module map.",
        ],
        acceptance=[
            "A remote request without full control that changes sandbox, tools, or body gets 403 and writes nothing.",
            "Concurrent profile saves keep both edits; oversized uploads get 413.",
            "A failed boot stops workers it already started.",
        ],
        evidence=[
            ("Remote policy gate tests", "tests/web-workspace-security.test.ts"),
            ("Composition-root tests", "tests/daemon-runtime-composition.test.ts"),
        ],
        out_of_scope=["Inference gateway lifecycle, stop mid-boot, usage single-flight, and the attachment cleanup timer need a real OMP session or long timers and stay untested through bootDaemon."],
    ),
    Task(
        id="T-1804", slug="website-parity-and-ci", title="Website claims, demo parity, CI gates, and web-next removal",
        epic="EP-18", sprint="SP-19", status="Done",
        goal="web-next is gone. CI builds and typechecks the website, runs the demo mock smoke, lints web/, and runs a parity test that fails when the daemon's console routes or frames drift from the mock. The mock answers like the daemon, and the site's claims match the product.",
        read_first=[
        ...
        assets=[
            (".github/workflows/ci.yml", "Edited", "Website install, typecheck, build, mock smoke, and web lint; no web-next."),
            (".github/workflows/release.yml", "Edited", "Same gates as CI; no web-next."),
            (".gitignore", "Edited", "No web-next build output."),
            ("README.md", "Edited", "Website and demo note."),
            ("biome.json", "Edited", "No web-next ignore."),
            ("docs/web-console.md", "Edited", "Server-rendered console section removed."),
            ("package.json", "Edited", "No console:next scripts or web-next typecheck."),
            ("tests/console-next.test.ts", "Edited", "Deleted with web-next."),
            ("tests/website-mock-parity.test.ts", "Edited", "Fails when the daemon gains a console route or frame the mock lacks."),
            ("tsconfig.json", "Edited", "No web-next exclude."),
            ("web/.oxlintrc.json", "Edited", "exhaustive-deps as a warning."),
            ("website/README.md", "Edited", "Six themes; demo parity notes."),
            ("website/bun.lock", "Edited", "Locked with the console's direct dependencies."),
            ("website/next.config.ts", "Edited", "No dead Turbopack block."),
            ("website/package.json", "Edited", "Declares the console dependencies the build compiles."),
            ("website/scripts/mock-smoke.mjs", "Edited", "Fails on unknown routes, unexpected shapes, and a changed frame set; covers every paging mode."),
            ("website/vercel.json", "Edited", "Installs with Vercel's own Bun."),
            ("src/shared/agent-state.ts", "New", "AgentState with no imports, so the demo mock typechecks without OMP installed."),
            ("src/shared/protocol.ts", "Edited", "Re-exports AgentState from agent-state.ts."),
            ("website/src/app/(home)/home.css", "Edited", "Contrast and focus fixes."),
            ("website/src/app/(home)/layout.tsx", "Edited", "Canonical link."),
            ("website/src/app/(home)/responsive.css", "Edited", "Footer heading styles."),
            ("website/src/app/console/console.css", "Edited", "Stale Schedules scroll workaround removed."),
            ("website/src/app/console/layout.tsx", "Edited", "Demo layout metadata."),
            ("website/src/app/layout.tsx", "Edited", "metadataBase from the deployment URL before localhost."),
            ("website/src/components/demo/ConsoleApp.tsx", "Edited", "Uninstalls the mock transport on unmount; no __omaAsset."),
            ("website/src/components/home/data.ts", "Edited", "Theme count from the source; models in the verb list."),
            ("website/src/components/home/hero/Hero.tsx", "Edited", "Demo wording."),
            ("website/src/components/home/hero/HeroCanvas.tsx", "Edited", "Releases probe contexts; context-lost fallback."),
            ("website/src/components/home/sections/Clients.tsx", "Edited", "Six themes."),
            ("website/src/components/home/sections/DemoTeaser.tsx", "Edited", "Accessible link names."),
            ("website/src/components/home/sections/Footer.tsx", "Edited", "Heading order."),
            ("website/src/components/home/sections/Isolation.tsx", "Edited", "Accessibility audit fix."),
            ("website/src/components/home/sections/Remote.tsx", "Edited", "Tailscale recipe verified; origin required only in remote mode."),
            ("website/src/components/home/sections/Transcript.tsx", "Edited", "Accessibility audit fix."),
            ("website/src/components/home/three/Scene.tsx", "Edited", "Context-lost handling."),
            ("website/src/components/home/threeui/ThreeStage.tsx", "Edited", "Releases WebGL contexts on unmount."),
            ("website/src/components/home/ui/chrome.tsx", "Edited", "Demo wording and link names."),
            ("website/src/mock/activity.ts", "Edited", "Tracked greeting timer; throttled persistence while streaming."),
            ("website/src/mock/fixtures/agents.ts", "Edited", "No account fixtures."),
            ("website/src/mock/fixtures/index.ts", "Edited", "No account fixtures."),
            ("website/src/mock/index.ts", "Edited", "Transport uninstall export."),
            ("website/src/mock/messages.ts", "Edited", "Daemon paging."),
            ("website/src/mock/router.ts", "Edited", "404 for unknown routes; exported route table."),
            ("website/src/mock/routes/agents.ts", "Edited", "Stopped-agent and definition-only statuses; keptChildren."),
            ("website/src/mock/routes/rooms.ts", "Edited", "Plan errors, message paging, and channel frames as the daemon sends them."),
            ("website/src/mock/routes/system.ts", "Edited", "Image avatars; no bump route."),
            ("website/src/mock/routes/workspace.ts", "Edited", "truncated on inspection; upload size cap."),
            ("website/src/mock/store.ts", "Edited", "Validated stored state and re-seeded timestamps."),
            ("website/src/mock/transport.ts", "Edited", "Uninstallable patches; upload size cap."),
            ("website/src/mock/types.ts", "Edited", "Frame type follows the daemon."),
            ("web-next/.gitignore", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/README.md", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/bun.lock", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/next.config.ts", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/package.json", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/postcss.config.mjs", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/agents/page.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/api/[...path]/route.ts", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/api/live/route.ts", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/artifacts/page.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/error.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/favicon.ico", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/globals.css", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/layout.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/page.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/rooms/[id]/page.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/app/rooms/[id]/plans/page.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/components/Action.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/components/Composer.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/components/Live.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/components/Markdown.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/components/Mermaid.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/components/Reactions.tsx", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/lib/daemon.ts", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/src/lib/types.ts", "Edited", "Deleted with web-next (ADR-018)."),
            ("web-next/tsconfig.json", "Edited", "Deleted with web-next (ADR-018)."),
        ],
        steps=[
            "Delete web-next and every reference to it.",
            "Gate the website and mock in CI and add the parity test.",
            "Match the mock to the daemon and correct the site's claims.",
        ],
        acceptance=[
            "CI runs the website build, the mock smoke, and the parity test on every push and pull request.",
            "The mock answers unknown routes with 404 and pages messages like the daemon.",
            "The homepage states six themes and a verified tailscale recipe.",
        ],
        evidence=[
            ("Mock parity test", "tests/website-mock-parity.test.ts"),
            ("Mock smoke", "website/scripts/mock-smoke.mjs"),
        ],
        depends_on=["T-1802"],
        out_of_scope=["A website-only install: Tailwind resolves web/src/index.css imports from web/, so Vercel installs both directories."],
    ),
    Task(
        id="T-1805", slug="console-view-fixes", title="Plans keep edits on conflict and console views stop misbehaving",
        epic="EP-18", sprint="SP-19", status="Done",
        goal="Refreshing a conflicted plan loads the server copy so a second save cannot overwrite another writer. The Changes view debounces and resets per room, an error boundary contains bad payloads, dialogs keep unsaved edits and report partial saves, remote consoles without full control lock policy fields, Storybook loads lazily, and its test renders every story in Chrome.",
        read_first=[
        ...
        assets=[
            ("web/src/console/PlansView.tsx", "Edited", "Refresh loads the server copy; conflict by error code; a vanished plan disables Save."),
            ("web/src/console/ChangesView.tsx", "Edited", "Debounced path, stale responses ignored, truncation notice."),
            ("web/src/console/ConsoleShell.tsx", "Edited", "Per-room Changes path, picker cancel sends nothing, error boundary, scoped stopped banner, encoded chat ids."),
            ("web/src/console/ErrorBoundary.tsx", "New", "Recoverable fallback for a view that throws."),
            ("web/src/console/ViewTabs.tsx", "New", "Tabs shared by the shell and Storybook."),
            ("web/src/console/errors.ts", "New", "Error text, error code check, and safe date formatting."),
            ("web/src/console/ArtifactsView.tsx", "Edited", "Safe dates; Open review opens its window on the click."),
            ("web/src/console/ProfileDialog.tsx", "Edited", "Draft seeded on open and kept across profile frames."),
            ("web/src/console/CreateAgentDialog.tsx", "Edited", "Required fields marked and explained; remote create needs full control."),
            ("web/src/console/DefinitionDialog.tsx", "Edited", "Partial saves reported; policy sections locked remotely without full control."),
            ("web/src/console/definition/identity-sections.tsx", "Edited", "Tools and spawn rights locked remotely without full control."),
            ("web/src/console/Composer.tsx", "Edited", "Read-only while sending, so focus and typed text survive."),
            ("web/src/console/AgentPanel.tsx", "Edited", "Instruction field has an id."),
            ("web/src/console/WorkspaceToolbar.tsx", "Edited", "Search name starts with its visible text."),
            ("web/src/console/ChannelRail.tsx", "Edited", "Workspace menu name starts with its visible text."),
            ("web/src/console/Message.tsx", "Edited", "Reaction pill names start with their visible text."),
            ("web/src/console/Storybook.tsx", "Edited", "Shares tabs with the shell."),
            ("web/src/App.tsx", "Edited", "Storybook loaded lazily."),
            ("storybook/console/serve.ts", "Edited", "Serves lazy chunks."),
            ("storybook/console/stories.js", "Edited", "Story fixtures for the new states."),
            ("tests/fixtures/chrome.ts", "New", "Shared headless Chrome launcher."),
            ("tests/console-storybook.test.ts", "Edited", "Renders every story in Chrome and fails on page or console errors."),
            ("tests/console-client.test.ts", "Edited", "Plan conflict, Changes, Artifacts, error boundary, and chat panel tests on a daemon-served workspace harness."),
        ],
        steps=[
            "Fix the plan conflict flow and cover it with two pages.",
            "Fix each view defect with a browser test that fails without it.",
            "Render Storybook stories in the suite.",
        ],
        acceptance=[
            "After a conflict and Refresh latest plan, saving keeps the other writer's edit visible and never silently overwrites it.",
            "Typing a path in Changes sends one inspection after typing stops.",
            "Every story renders in Chrome with no page or console errors.",
        ],
        evidence=[
            ("Plan conflict and view tests", "tests/console-client.test.ts"),
            ("Story render test", "tests/console-storybook.test.ts"),
        ],
        depends_on=["T-1801"],
        out_of_scope=["The Open review popup timing and the partial definition save are verified by reading, not by a browser test."],
    ),
]
...
TASKS += [
    ...
    Task(
        id="T-1616", slug="control-cap-json-rpc", title="A saturated audit stays readable on the control socket",
        epic="EP-16", sprint="SP-17", status="Done",
        goal="When the live connection audit is full, the control socket answers with a JSON-RPC failure frame carrying the daemon's own reason, so `omp-agent audit` -- the verb an operator reaches for at exactly that moment -- prints why instead of a JSON parser's complaint.",
        read_first=[
        ...
        files=["src/daemon/socket.ts", "tests/remote-exposure.test.ts"],
        assets=[
            ("src/daemon/socket.ts", "Edited", "The audit-capacity refusal returns a JSON-RPC failure frame through the existing `failure` helper rather than a bare-text body, matching what console-api.ts already returned for the same condition."),
            ("tests/remote-exposure.test.ts", "Edited", "Created by T-1201; a 33rd control connection at the cap is asserted to be a parseable JSON-RPC error, and `runCli(['audit'])` is asserted to print the daemon's reason and never 'Failed to parse JSON'."),
        ],
        steps=[
            "Reproduce against a real proxy: hold 32 authenticated console WebSockets open, then run `omp-agent audit` on the daemon host.",
            "Return the capacity refusal through the same `failure` helper every other control-socket error uses, keeping the 503 status and the existing message.",
            "Assert both surfaces: the wire frame parses as JSON-RPC with the daemon's message, and the CLI prints that message rather than a parser error.",
        ],
        acceptance=[
            "At the audit cap the control socket returns HTTP 503 whose body is a JSON-RPC failure frame carrying `Connection audit capacity reached`.",
            "`omp-agent audit` at the cap exits 4 and prints the daemon's reason, never `Failed to parse JSON`.",
        ],
        evidence=[
            ("The audit-capacity refusal returns a JSON-RPC failure frame", "src/daemon/socket.ts"),
            ("A control connection at the cap is a readable JSON-RPC error, CLI surface included", "tests/remote-exposure.test.ts"),
            ("Found by the 33rd-connection check of the 2026-09-03 real-proxy runs; the fix was re-verified through real Caddy at the cap", "docs/remote-exposure.md"),
        ],
    ...
    Task(
        ...
        assets=[
            ("src/shared/version.ts", "New", "The package version as one value; status reports it so a daemon older than its plugin says so."),
            ("src/extension/widget.ts", "Edited", "A token fault is DaemonAuthError, never absence; the unread count follows a real cursor instead of re-reading every room."),
            ("src/extension/ensure-daemon.ts", "Edited", "Spawns only on absence, bounds the wait, and surfaces the launcher's own reason."),
            ("src/daemon/runtime.ts", "Edited", "AlreadyRunningError answers a second launch cleanly; credential files heal; every shutdown step runs; start failures reach status."),
            ("src/daemon/scheduler.ts", "Edited", "Delays past the timer bound are chained, and impossible dates are refused before the scan."),
            ("src/daemon/supervisor.ts", "Edited", "A stopped peer is never rebuilt into a running one; a killed peer is unregistered from its account."),
            ("src/worker/lifecycle.ts", "Edited", "Sandbox read roots come from the executed artifacts; the launch shim records the worker pid for unpatched installs."),
            ("src/daemon/web-chats.ts", "Edited", "Chat liveness comes from a shim-recorded pid, so chats work without the repository's patch."),
            ("src/daemon/console-api.ts", "Edited", "Point reads replace full-history scans; loopback pages are uncached and refuse foreign WebSocket origins."),
            ("src/rooms/store.ts", "Edited", "latestMessageId and getMessage; pre-threading databases gain parent_id."),
        ],
        steps=[
            "Reproduce each defect from a real install's logs or a real code path, not from the suite, which passed throughout.",
            "Fix each at the shared function every caller routes through rather than at the one caller that surfaced it.",
            "Add a regression test per fix that drives production code, then revert the fixing line and confirm that exact test fails before restoring it.",
        ],
        acceptance=[
            "A missing operator token against a live socket is reported as an auth fault and never triggers a spawn; a second launch against a live daemon exits zero and names it.",
            "A killed peer stays stopped after its definition changes, and a cron more than 24.8 days out is armed once rather than in a loop.",
            "A worker on an install without the patch reports its pid, and a web chat there is reported running.",
            "A console post lands in a room with more history than any fixed read-back window, and a loopback WebSocket upgrade from a foreign origin is refused.",
        ],
        evidence=[
            ("Token fault is not absence; ensureDaemon does not spawn on it", "tests/extension.test.ts"),
            ("Second launch against a live daemon exits zero without a stack", "tests/daemon-cli.test.ts"),
            ("Token heals, status carries the version, shutdown survives a failing step", "tests/daemon-main.test.ts"),
            ("Killed peer is not rebuilt by a stale definition", "tests/supervisor.test.ts"),
            ("Long delays chain; impossible dates are refused quickly", "tests/scheduler.test.ts"),
            ("Shim-recorded worker pid; Linux sandbox acceptance", "tests/worker-lifecycle.test.ts"),
            ("Web chat liveness without the patched accessor", "tests/web-workspace-security.test.ts"),
            ("Long-history post and foreign-origin WebSocket refusal", "tests/console-api.test.ts"),
            ("Commit", "c5c78ed"),
            ("Commit", "aba7dd4"),
            ("Commit", "00225ce"),
            ("Commit", "7d88b66"),
