...
**49 of 55 tasks Done.** Suite state is not restated here, because a pasted count rots the day after it is pasted: CI runs `tsc --noEmit` and `bun test` on every push, and `bun test` locally gives you the same answer.
...
| [EP-07](epics/EP-07-release-readiness.md) | Release readiness: CI, lint, and a README a stranger can act on | Done | 4 |
...
| [EP-10](epics/EP-10-production-wiring.md) | Production wiring: serving, usage, and deferred hardening | Ready | 6 |
...
| [SP-11](sprints/SP-11-production-wiring.md) | Production wiring | Ready | The console served for real, budgets fed by real usage, and the hardening deferred to a named trigger. |
