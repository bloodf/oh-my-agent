# SP-15 — Live accounts

**Status:** Ready

*Derived from the tasks below.*

## Theme

Dogfooding against real accounts, with findings landing back in the tree as tasks.

## Tasks

| Task | Epic | Title | Status |
|---|---|---|---|
| [T-1401](../tasks/T-1401-dogfood-runbook.md) | [EP-14](../epics/EP-14-dogfooding.md) | Dogfooding runbook | Ready |
| [T-1402](../tasks/T-1402-dogfood-harness.md) | [EP-14](../epics/EP-14-dogfooding.md) | Scripted dogfood scenario driver | Blocked |
| [T-1403](../tasks/T-1403-first-live-session.md) | [EP-14](../epics/EP-14-dogfooding.md) | First live session and triage | Blocked |
