...
1. Author the files allowlist: src, skills, patches, LICENSE, README — skills/ is load-bearing (the materializer throws Unknown skill without it) and is in the manifest today. Tests, docs, and .github stay out. patches/ stays in the allowlist for developers installing from a checkout; whether npm actually packs the patched file varies by npm version, and the test treats both outcomes as acceptable.
