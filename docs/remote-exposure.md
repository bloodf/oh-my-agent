...
Remote browser access is supported. The browser keeps the operator token in `sessionStorage` and sends it only in request headers. For proxied remote traffic, `console-api.ts` checks that token on `/api/*` HTTP requests, including `POST /api/session` and `POST /api/ws-ticket`. Successful session authentication mints a ticket bound to `/`; the authenticated shell sets one reusable asset pass as a `Secure`, `HttpOnly`, `SameSite=Strict` cookie (twelve-hour TTL) that opens `/app.js`, `/style.css`, and lazily imported chunks such as the mermaid renderer, and nothing else: not `/`, not `/api/*`, not the WebSocket, and never as a query ticket. The shell loads `app.js` by its bare URL because chunks import it by relative URL; successful WebSocket-ticket authentication mints a ticket bound to `/api/events`. The shell load and the WebSocket upgrade present those tickets, and asset loads the pass cookie, instead of the operator token. Every ticket has a 30-second TTL, and `consumeTicket` deletes it before checking its path or expiry, so first presentation consumes it even when refused. `app.js` removes the root ticket from browser history. A remote API 401 revokes browser session: `app.js` blocks later API work and reconnects, closes live WebSockets, and invokes `__showOperatorAuth`, which clears stored token and returns to token entry. `tests/console-client.test.ts` checks token-free browser URLs and session revocation; `tests/remote-exposure.test.ts` checks server-side token and ticket boundaries.
...
| Transport outside host | Network observer reads or alters console traffic. | External proxy must terminate HTTPS and inject proxy secret; daemon stays loopback-only and implements no TLS. | Focused suite checks daemon loopback bind and proxy-secret gate only. All three usable recipes are additionally verified end to end from a separate operator machine: the Caddy and SSH-tunnel recipes against real Caddy-terminated TLS, the `tailscale serve` recipe against real tailnet HTTPS on two tailnet devices (see [Real-proxy evidence](#real-proxy-evidence)); public ACME issuance remains `UNVERIFIED`. |
...
`bind 127.0.0.1` keeps Caddy off every other interface: without it Caddy listens on all of them, and anyone on the LAN can reach the daemon over plaintext with the proxy secret injected. The site address has no host because `tailscale serve` forwards the tailnet `Host` header, which a `127.0.0.1` site would never match. `trusted_proxies` lets Caddy pass on the `X-Forwarded-For` that `tailscale serve` sets from the real tailnet peer; without it the audit records `127.0.0.1` for every operator. It also means any local process that reaches Caddy's loopback port can choose the recorded source, which is no more than host-local trust already grants. If the host already publishes something on tailnet port 443, publish on another port with `tailscale serve --bg --https=<port> http://127.0.0.1:8443` and include that port in `OMA_CONSOLE_ORIGIN`. Changing serve config needs root unless an operator was set once with `sudo tailscale set --operator=$USER`.
...
All three usable recipes are verified end to end against a real proxy. What is not verified is public ACME issuance and renewal for the Caddy recipe (see the deviation below).

| Recipe | Date | Version | Result |
|---|---|---|---|
| Caddy with public TLS | 2026-09-03 | Caddy v2.11.4 with `caddy-ratelimit` (`xcaddy` build), Chromium 151, Bun 1.3.14 | PARTIAL — 11/11 checks on an internal CA; public ACME issuance and renewal remain UNVERIFIED (see deviation below) |
| `tailscale serve` | 2026-09-13 | Tailscale 1.102.3 (both devices), Caddy v2.11.4 with `caddy-ratelimit` (`xcaddy` build), Chromium 152, Bun 1.3.14 (daemon host) and 1.4.2 (operator) | PASS — 11/11 checks, after the two recipe fixes described below |
| SSH tunnel with loopback Caddy | 2026-09-03 | Caddy v2.11.4 with `caddy-ratelimit`, OpenSSH_10.0p2 (client and server), Chromium 151, Bun 1.3.14 | PASS — 11/11 checks |
| SSH-only tunnel | REJECTED — no external HTTPS origin | N/A | Remote console startup refuses this configuration |

The Caddy and SSH-tunnel runs used two separate machines — a daemon host and an operator machine with no route to the daemon's loopback — on an isolated Docker bridge network. Each ran real Caddy built with the real `rate_limit` handler, terminating real TLS, with the CA trusted in the operator machine's system and browser trust stores, and drove a real Chromium against it.

The `tailscale serve` run used two real devices on one tailnet: an Ubuntu 26.04 x86_64 daemon host running the daemon, the loopback Caddy boundary above (same `xcaddy` build), and `tailscale serve`; and a macOS operator machine reaching it only through the tailnet. `tailscale serve` terminated TLS with the tailnet's Let's Encrypt certificate, which Chromium and Bun each validated through their default trust stores with no override. The host already published another service on tailnet port 443, so the run used `--https=8443` and `OMA_CONSOLE_ORIGIN=https://<host>.<tailnet>.ts.net:8443`, and wrote the access log to a temporary directory instead of `/var/log/caddy`. `omp-agent console` printed exactly that origin.

Each recorded, per recipe: a trusted HTTPS handshake with no certificate override; a 401 token-entry bootstrap for an anonymous shell, stripped of `app.js` and `style.css`; a 401 for anonymous `/api/*`; an operator token minting a root ticket; a wrong token refused; that ticket serving the shell exactly once and refused on replay; the browser prompt, keyboard login, and `#operator-auth[hidden]`; a final URL with no `token=`, no `ticket=`, and no operator token; and no connection-failure banner. On the daemon side, `omp-agent audit` showed exactly one live `operator` / `console-proxied` row with the forwarded client address while the browser WebSocket was open, and zero after close — which also shows the proxy's forwarded metadata was accepted, since the class is `console-proxied` only when `X-OMA-Proxy-Secret` matched and the forwarded scheme and host were honored, and any of those missing would have recorded `console-loopback`. Caddy's access log recorded 208 (Caddy recipe), 17 (SSH recipe), and 235 (`tailscale serve` recipe) requests with zero occurrences of `Authorization` or `X-Operator-Token`. The rate-limit stanza refused rapid requests with 429: 47 of 150 in the Caddy and SSH runs, 31 of 150 in the `tailscale serve` run, which started that burst on a fresh one-minute window. A 33rd concurrent authenticated connection was refused with 503 while the audit held exactly 32. At the cap `omp-agent audit` is itself refused, exiting 4 with `Connection audit capacity reached`, because its control-socket connection counts too; the `tailscale serve` run therefore read the 32 from `connection-audit.json`, the state `omp-agent audit` reports.

That 33rd-connection check found a real defect, now fixed: the control socket answered a saturated audit with a bare-text 503 body, so `createCliClient` — which parses every reply as JSON-RPC — failed with `Failed to parse JSON`. An operator running `omp-agent audit` to diagnose a saturated audit got a parser's complaint instead of the daemon's reason. The control socket now returns a JSON-RPC failure frame carrying the same reason; the console API's own 503 was already parseable JSON, in that surface's REST-shaped envelope.

The `tailscale serve` run found two defects in that recipe as it was then written, both fixed above. First, its site address was `http://127.0.0.1:8443` with no `bind`: Caddy listened on every interface, a tailnet request to `/api/x` got an empty 200 from Caddy instead of the daemon's 401, because the forwarded tailnet `Host` never matched the site, and a LAN client that sent `Host: 127.0.0.1:8443` to the host's LAN address reached the daemon over plaintext with the proxy secret injected. With `http://:8443` and `bind 127.0.0.1`, tailnet requests reached the daemon and the same LAN request was refused at connect. Second, Caddy discarded the `X-Forwarded-For` that `tailscale serve` set, so the live audit row read `127.0.0.1`; with `trusted_proxies` it read the operator's tailnet address. A tailnet client that sent its own `X-Forwarded-For: 203.0.113.7` was still recorded under its real tailnet address, because `tailscale serve` replaces that header.

**Deviation, stated plainly.** The Caddy and SSH-tunnel runs used Caddy's `tls internal` rather than a public ACME certificate on public DNS, because this lab has no inbound public DNS name to bind. `tls internal` is a real CA issuing a real certificate that a real browser validates through its real trust store, so the TLS transport, the proxy boundary, the header injection, and the browser flow are genuinely exercised. What these runs do **not** establish is public ACME issuance or renewal, or behavior on the public internet.

To close the remaining `PARTIAL` cell, run the Caddy recipe on a host with a public DNS name and inbound TCP 443, let Caddy obtain a public ACME certificate, repeat the checks above, and record UTC date, `caddy version`, and result. The `tailscale serve` run does not cover this: its certificate came from `tailscale serve`, not from Caddy's ACME client.
