...
| Transport outside host | Network observer reads or alters console traffic. | External proxy must terminate HTTPS and inject proxy secret; daemon stays loopback-only and implements no TLS. | Focused suite checks daemon loopback bind and proxy-secret gate only. The Caddy and SSH-tunnel recipes are additionally verified end to end against real Caddy-terminated TLS from a separate operator machine (see [Real-proxy evidence](#real-proxy-evidence)); public ACME issuance and the `tailscale serve` row remain `UNVERIFIED`. |
...
Two recipes are verified end to end against a real proxy. The `tailscale serve` recipe is not: it needs two tailnet devices, and no second device on this tailnet accepted a shell, nor was an auth key available to enlist one.
...
| `tailscale serve` | UNVERIFIED — no second tailnet device available | UNVERIFIED — no second tailnet device available | UNVERIFIED — no second tailnet device available |
...
**Deviation, stated plainly.** Both runs used Caddy's `tls internal` rather than a public ACME certificate on public DNS, because this lab has no inbound public DNS name to bind. `tls internal` is a real CA issuing a real certificate that a real browser validates through its real trust store, so the TLS transport, the proxy boundary, the header injection, and the browser flow are genuinely exercised. What these runs do **not** establish is public ACME issuance or renewal, or behavior on the public internet.

To fill the remaining `UNVERIFIED` row, the operator must run that recipe on real infrastructure and record UTC date, exact tool version, and result:

1. **`tailscale serve`:** run Caddy boundary and `tailscale serve` from two tailnet devices; repeat TLS, login, URL, WebSocket, source, cap, and close checks. Record `tailscale version` and `caddy version`.
